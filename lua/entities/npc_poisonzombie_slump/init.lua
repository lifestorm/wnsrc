...
function ENT:SpawnFunction( ply, tr )
	...
	local ent = ents.Create( "npc_poisonzombie_slump" )
	ent:SetPos( SpawnPos )
	ent:SetModel( "" )
...
function ENT:Initialize()
	self.zombie = ents.Create( "npc_poisonzombie" )
	...
	self.zombie_seq:SetKeyValue( "m_iszPlay", "slumprise_a" )
...
function ENT:Think()
	...
	for k, v in pairs( ents.FindInSphere( self.zombie:GetPos(), 64 ) ) do
		if v and IsValid( v ) and v:IsPlayer() then
			...
			if rand == 1 then
				self.zombie:EmitSound( "npc/zombie_poison/pz_alert1.wav", 100, 100 )
			elseif rand == 2 then
				self.zombie:EmitSound( "npc/zombie_poison/pz_alert2.wav", 100, 100 )
			end
