...
local PLUGIN = ix.plugin.Get("xenforolink")
...
function PLUGIN:FindUser(client, name)
    ...
    if (string.find(name, "wnveles.pl/members/[^%.]+%.%d+/")) then
        PLUGIN:CheckLinkExists(client, string.match(name, "wnveles.pl/members/[^%.]+%.(%d+)/"))
        return
    ...
    local endpoint = "https://wnveles.pl/api/users/find-name?username="
    if (string.find(name, "@")) then
        endpoint = "https://wnveles.pl/api/users/find-email?email="
    end
...
function PLUGIN:StartLink(client, forumID)
    ...
    local endpoint = string.format("https://wnveles.pl/api/conversations/")
    local request = {
        failed = function(error)
