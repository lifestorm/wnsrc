...
function PLUGIN:F1MenuCreated(panel, mainFrame, leftFrame, rightFrame)
	...
	notes:SetText("Notes personnelles")
	notes:Dock(TOP)
...
function PLUGIN:CreateNotesTab(characterId, charNotes)
	...
	frame:SetTitle(characterId != nil and "Notes STAFF" or "Notes personnelles")
	frame:Center()
	frame:MakePopup()
	...
	if (!text) then
		frame:Remove()
		LocalPlayer():Notify("Impossible d'ouvrir les notes de personnage")
	...
	submitButton:SetText("Confirmer")
	submitButton.DoClick = function(self)
		if (characterId) then
			...
			LocalPlayer():Notify("Notes enregistrées avec succès.")
		end
	end
end
