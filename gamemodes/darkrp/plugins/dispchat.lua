...
ix.lang.AddTable("english", {
	dispOnlyChat = "Dispatch komut sohbeti, sadece diğer dispatchler, scannerlar ve OTA'lar bunu duyacak.",
	dispAllCombine = "Combine ve Sivil Koruma birimlerine önemli mesajlar iletmek için kullanılır."
})
			...
			if (!speaker:IsDispatch() and !speaker:IsOTA() and speaker:Team() != FACTION_SERVERADMIN) then
				...
				if (delay > 0 and speaker.ixLastDispatchOOC) then
					...
					if (lastOOC <= delay and !CAMI.PlayerHasAccess(speaker, "Helix - Bypass OOC Timer", nil)) then
						speaker:Notify("Dispatch OOC'yi kullanmadan önce " .. delay - math.ceil(lastOOC) .. " saniye beklemelisiniz!")
	...
	ix.command.Add("D", {
		...
		OnRun = function(self, client, message)
			if (ix.config.Get("allowDispatchOOC", true) or client:IsAdmin()) then
				...
				client:Notify("Dispatch OOC sohbeti şu anda devre dışı!")
			end
		end,
		...
		CanSay = function(self, speaker, text)
			if (!speaker:IsDispatch() and !speaker:IsOTA() and speaker:Team() != FACTION_SERVERADMIN) then
				...
				if (delay > 0 and speaker.ixLastCcgOOC) then
					...
					if (lastOOC <= delay and !CAMI.PlayerHasAccess(speaker, "Helix - Bypass OOC Timer", nil)) then
						speaker:Notify("CCG OOC'yi kullanmadan önce " .. delay - math.ceil(lastOOC) .. " saniye beklemelisiniz!")
	...
	ix.command.Add("CCG", {
		...
		OnRun = function(self, client, message)
			if (ix.config.Get("allowCcgOOC", true) or client:IsAdmin()) then
				...
				client:Notify("CCG OOC sohbeti şu anda devre dışı bırakıldı!")
			end
		end,
		OnCheckAccess = function(self, client)
			return client:IsDispatch() or client:IsOTA() or client:Team() == FACTION_SERVERADMIN
