...
function PLUGIN.AssignApartment(idCard, genericData, client, foundApartment)
	if idCard:GetCredits() < ix.config.Get("costForAnApartment", 35) then
		netstream.Start(client, "SendHousingErrorMessage", "YETERLİ KREDİ YOK")
		return
	end

	if !foundApartment then
		netstream.Start(client, "SendHousingErrorMessage", "KONUT BULUNAMADI")
		return
	end
	...
	if character then
		if !character:GetPurchasedItems()["apartmentkey"] then
			...
			client:NotifyLocalized("Bu CID için kasada bekleyen bir anahtar zaten var. Güncellendi.")
		end
	end
