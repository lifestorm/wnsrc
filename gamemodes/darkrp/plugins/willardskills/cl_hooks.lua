...
function PLUGIN:PopulateHelpMenu(tabs)
	...
	if (permitsList and !table.IsEmpty(permitsList)) then
		tabs["İzinler"] = function(container)
			...
			for _, v in pairs(permits) do
				...
				permitsButton.DoClick = function()
					LocalPlayer():NotifyLocalized("Permit ID '"..v.."' panoya kopyalandı")
					SetClipboardText( v )
				end
			end
		end
...
function PLUGIN:CreateExtraCharacterTabInfo(character, informationSubframe, CreatePart)
	...
	if !istable(permitsData) then
		CreatePart(permitsPanel, "İş İzinleri:", "İzin yok", "licenses", true, false, true)
		return
	end
	...
	if !table.IsEmpty(permitsData) then
		CreatePart(permitsPanel, "İş İzinleri:", table.concat(permitsData, ", "), "licenses", true, false, true)
	else
		CreatePart(permitsPanel, "İş İzinleri:", "İzin yok", "licenses", true, false, true)
	end
end
...
function PLUGIN:AdjustInnerStatusPanel(innerStatus, CreateTitle, CreateSubBar)
	if (ix.special) then
		...
		CreateTitle(attributesTitle, "NITELIKLER")
	...
	if (ix.skill) then
		...
		CreateTitle(skillsTitle, "BECERILER")
