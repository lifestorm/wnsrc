	...
	net.Receive("ixCharacterSkill", function()
		...
		if (character) then
			...
			if (IsValid(ix.gui.skills)) then
				ix.gui.skills.skillPanels[skill].skillLevel:SetText("Niveau : "..character:GetSkillLevel(skill).."/"..ix.skill.MAX_SKILL)
			end
		end
	end)
...
function ix.skill:RegisterSkill(uniqueID, data)
	if (!uniqueID or type(uniqueID) != "string") then
		ErrorNoHalt("Tentative d'enregistrement d'une compétence sans ID.")
		return
	end
	...
	data.description = data.description or "Aucune description."
...
function ix.skill:RegisterSkillScale(skillID, uniqueID, data)
	...
	data.name = data.name or "Inconnu"
	data.description = data.description or "Aucune description."
