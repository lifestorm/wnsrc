...
ix.lang.AddTable("english", {
	ffLostInitiative = "İnisiyatif kaybettiniz! Bir tur daha geç oynayacaksınız.",
	ffGainedInitiative = "İnisiyatif kazandınız! Bir tur daha erken oynayacaksınız.",
	levelGained = "%s yeteneğinizde bir seviye kazandınız ve şu an %d. seviyedesiniz.",
	levelPossible = "%s yeteneğinizde %d. seviyeye çıkabilirsiniz.",
	cmdCharSkillSet = "Belirtilen karakterin verilen yetenekteki seviyesini ayarlayın.",
	cmdCharSkillAdd = "Bir karakterin yeteneğine belirtilen miktarda seviye ekleyin.",
	cmdCharSkillReset = "Bir karakterin yeteneğini 0 seviyesine sıfırlayın.",
	cmdCharSkillResetAll = "Bir karakterin tüm yeteneklerini 0 seviyesine sıfırlayın.",
	cmdRefundSkills = "Oyuncuya son ölümünden sonra kaybettiği tüm yetenek seviyelerini geri verir.",
	assembleCant = "Bunu monte edebilmeniz için beklemeniz gerekiyor!",
	assembleMissingRequirement = "Bu öğeyi monte etmek için bir %s'e ihtiyacınız var!",
	assembleSuccess = "Bir %s monte ettiniz.",
	assembleNoSpace = "Bu öğeyi monte etmek için %sx%s envanter alanına ihtiyacınız var!"
})
  ...
  OnRun = function(self, client, character)
		if (character) then
			...
			if (table.IsEmpty(permits)) then
				client:ChatPrint(character:GetName().." adlı kişinin izni yoktur.")
			else
				client:ChatPrint(character:GetName().." şu izinlere sahiptir:")
...
function PLUGIN:InitializedPlugins()
	...
	for uniqueID, ITEM in pairs(items) do
		if (ITEM.action) then
			ix.action:RegisterSkillAction(ITEM.action.skill, "item_"..uniqueID, {
				name = "Use "..ITEM.name,
				description = "'"..ITEM.name.."' eşyasını kullanmanızı sağlar.",
	...
	for _, RECIPE in pairs(recipes) do
		if (RECIPE.skill == "bartering") then
			ix.action:RegisterSkillAction(RECIPE.skill, "recipe_"..RECIPE.uniqueID, {
				...
				description = "'"..RECIPE.name.."' eşyasını almanızı sağlar.",
		...
		elseif (RECIPE.level and !RECIPE.bNoAction) then
			ix.action:RegisterSkillAction(RECIPE.skill, "recipe_"..RECIPE.uniqueID, {
				...
				description = "'"..RECIPE.name.."' tarifini oluşturmanızı sağlar.",
				CanDo = RECIPE.level,
				DoResult = RECIPE.experience
			 })
		end
	...
	for k, v in pairs(ix.weapons.weaponCatList) do
		ix.skill:RegisterSkillScale("guns", "familiarity_"..k, {
			name = "Silah Aşinalığı: "..v.name,
			description = "Bu kategorideki silahları ne kadar iyi kullanabileceğinizi belirler.",
			minLevel = 0,
			maxLevel = 50,
