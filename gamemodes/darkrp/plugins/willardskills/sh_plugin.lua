...
ix.util.IncludeDir("darkrp/plugins/willardskills/recipes", true)
...
ix.config.Add("DeathSkillsReducing", 0, "Combien d'expérience de compétence (en pourcentage) le personnage perdra après sa mort (0 pour garder les compétences inchangées)..", nil, {
	data = {min = 0, max = 100, decimals = 0},
	category = "Compétences"
})

ix.config.Add("MaxTotalSkill", 120, "Le niveau maximum de compétences combinées qu'une personne peut avoir.", nil, {
	data = {min = 50, max = 300, decimals = 0},
	category = "Compétences"
})

ix.lang.AddTable("english", {
	ffLostInitiative = "Vous avez perdu l'initiative ! Vous jouerez un tour plus tard",
	ffGainedInitiative = "Vous avez gagné l'initiative ! Vous jouerez un tour plus tôt",
	levelGained = "Vous avez gagné un niveau dans votre compétence %s pour atteindre le niveau %d.",
	levelPossible = "Vous pouvez monter de niveau dans votre compétence %s jusqu'au niveau %d.",
	cmdCharSkillSet = "Fixer le niveau du personnage spécifié dans la compétence donnée",
	cmdCharSkillAdd = "Ajoute le nombre de niveaux indiqué à la compétence d'un personnage",
	cmdCharSkillReset = "Remettre la compétence d'un personnage à 0",
	cmdCharSkillResetAll = "Remettre toutes les compétences d'un personnage à 0",
	cmdRefundSkills = "Retourne toutes les compétences perdues par le joueur après sa dernière mort",
	assembleCant = "Vous devez attendre avant de pouvoir assembler ceci !",
	assembleMissingRequirement = "Vous avez besoin d'un %s pour assembler cet objet !",
	assembleSuccess = "Vous avez assemblé un %s.",
	assembleNoSpace = "Vous avez besoin de %sx%s d'espace d'inventaire pour assembler cet objet !"
})
...
function PLUGIN:InitializedPlugins()
	...
	for _, RECIPE in pairs(recipes) do
		if (RECIPE.skill == "bartering") then
			ix.action:RegisterSkillAction(RECIPE.skill, "recipe_"..RECIPE.uniqueID, {
				name = "Acheter "..RECIPE.name,
				description = "Permet d'acheter : '"..RECIPE.name.."'.",
		...
		elseif (RECIPE.level and !RECIPE.bNoAction) then
			ix.action:RegisterSkillAction(RECIPE.skill, "recipe_"..RECIPE.uniqueID, {
				name = "Fabriquer "..RECIPE.name,
				description = "Vous permet de fabriquer '"..RECIPE.name.."'.",
				CanDo = RECIPE.level,
				DoResult = RECIPE.experience
			 })
		end
	...
	for k, v in pairs(ix.weapons.weaponCatList) do
		ix.skill:RegisterSkillScale("guns", "familiarity_"..k, {
			name = "Familiarité avec les armes : "..v.name,
			description = "Détermine votre capacité à manier les armes de cette catégorie.",
			minLevel = 0,
			maxLevel = 50,
