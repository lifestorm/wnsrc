...
function PLUGIN:HandlePlayerDeath(victim, damage)
    if (victim.beingAmputated and victim.beingAmputatedEnd > CurTime() and damage:GetAttacker() == victim.beingAmputated) then
        ...
        victim.beingAmputated:Notify("Karar uygulandı!")
...
function PLUGIN:CanInsertDisk(client, item)
    ...
    if !targetEnt then client:Notify("Bir bilgisayara bakmıyorsunuz!") return false end
    if !IsValid(targetEnt) then client:Notify("Bir bilgisayara bakmıyorsunuz!") return false end
    local entClass = targetEnt:GetClass()
    if (entClass != "ix_medical_computer" and entClass != "ix_computer") then client:Notify("Bir bilgisayara bakmıyorsunuz!") return false end
    if targetEnt.hasDiskInserted then client:Notify("Bu bilgisayarda zaten bir disk var!") return false end
...
function PLUGIN:InsertDisk(client, item)
    ...
    if (!bSuccess and isstring(error)) then
        client:Notify("Disk bırakılamadı ve takılamadı.")
        return
    end
...
netstream.Hook("FindLetterRecepient", function(client, text, itemID)
    ...
    timer.Simple(3, function()
        if !id or !name or !cid then client:Notify("Mektup göndermeye çalıştığınız kişi bulunamadı.") return end
        cid = cid or "00000"
...
netstream.Hook("SendLetterToID", function(client, charID, itemID, fromCID, fromName)
    ...
    if ix.char.loaded[tonumber(charID)] then
        ...
        client:Notify("Mektup başarıyla gönderildi.")
