...
if SERVER then
	...
	function ENT:Use(user)
		...
		if self:GetNetVar("destroyed", false) == true and !item then
			user:Notify("Su kaynağında bir sızıntı var! Düzeltmek için bir alet takımına ihtiyacınız var.") -- message when dont work and you dont have item
			return false
		else
			if item and self:GetNetVar("destroyed", false) == true then
				...
				user:SetAction("Repairing...", 12, function() -- line with text when you repair it
					...
					if (item) then
						...
						self:Remove()
						user:Notify("Boru hattını onardınız.") -- message when you finish the job
					end
				end)
			end
		end
