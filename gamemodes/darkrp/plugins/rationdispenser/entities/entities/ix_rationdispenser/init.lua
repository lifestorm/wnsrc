...
function ENT:Use(client)
	...
	if isVort and character:GetBackground() == "İşbirlikçi" then
		isVort = false
	end
...
function ENT:IDCardCheck(idCard, genericData, client, entity)
	local character = client:GetCharacter()
	if client:IsVortigaunt() and character:GetBackground() != "İşbirlikçi" then
	...
	timer.Simple(math.random(1.8, 2.2), function()
		if (idCard:GetData("nextRationTime", 0) >= 60 * ix.config.Get("rationInterval")) then
			if !client:GetCharacter():GetInventory():GetItemByID(idCard.id) then
				if !client:IsVortigaunt() or character:GetBackground() == "İşbirlikçi" then
					return false
				end
			end
			...
			if (character:IsVortigaunt()) then
				if (genericData.cid or character:GetBackground() == "İşbirlikçi") then
			...
			client:NotifyLocalized("Bir sonraki rasyon alımınız "..math.Round( ((ix.config.Get("rationInterval", 0) * 60) - idCard:GetData("nextRationTime", 0)) / 60, 2).." saat sonra olacaktır.")
		end
	end)
end
