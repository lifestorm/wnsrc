...
function ENT:Use(activator, caller)
	if (activator:IsPlayer() and activator:GetEyeTraceNoCursor().Entity == self) then
		...
		if (istable(char)) then
			...
			if (cooldown > 0) then
				activator:NotifyLocalized("Nie możesz przeszukiwać śmieci przez następne "..tostring(cooldown).." sekund.")
				return
			end
		end
		if (activator:Crouching()) then
			if (!self.alive) then
				activator:NotifyLocalized("Ktoś inny już sprząta śmieci!")
				return
			else
				self.alive = false
			end
			...
			activator:SetAction(
				"Przeszukujesz śmieci......", trashSearchTime, function()
					...
					if (chance <= required) then
						...
						activator:NotifyLocalized("Nie udało Ci się nic znaleźć.")
					end
				...
				if (IsValid(activator)) then
					if (IsValid(self) and IsValid(self:GetPhysicsObject())) then
						if (self:GetPos():DistToSqr(activator:GetPos()) > 2500 or !activator:Crouching()) then
							...
							activator:NotifyLocalized("Odszedłeś od śmieci i przestałeś szukać.")
					...
					elseif (activator.searchingGarbage) then
						...
						activator.searchingGarbage = nil
						timer.Remove(uniqueID)
						activator:NotifyLocalized("Śmieci zostały usunięte, zanim zdążyłeś zakończyć przeszukiwanie.")
			...
			activator:NotifyLocalized("Musisz kucnąć, aby przeszukać śmieci.")
		end
	end
end
