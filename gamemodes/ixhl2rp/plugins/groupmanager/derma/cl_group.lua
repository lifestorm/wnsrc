...
function PANEL:CreateCreationPanel()
	...
	self:CreateTitleLabel(title, string.utf8upper("utworzenie grupy"))

	-- Create info text
	local infoText = self.contentFrame:Add("DLabel")
	self:CreateLabel(infoText, "Narzędzie to umożliwia łączenie graczy w grupy. Będziesz mieć dostęp do zakładki info\n listy członków, zakładki wiedzy i konfigurowalnych ról. Nie można należeć do wielu grup, tylko do jednej.", Color(230, 230, 230, 255))

	-- Create warning text
	local warningText = self.contentFrame:Add("DLabel")
	self:CreateLabel(warningText, "Należy pamiętać, że nazwy nieprzyjazne dla lore będą zmienione i mogą skutkować banem.", Color(255, 78, 69, 255), true)

	-- Create textentry
	self.textEntry = self.contentFrame:Add("DTextEntry")
	self:CreateTextEntry(self.textEntry, true, infoText:GetTall(), false, false, false, "Wpisz tutaj nazwę swojej grupy...")

	-- Create sign up amount requirement text
	local yellowText = self.contentFrame:Add("DLabel")
	self:CreateWarningText(yellowText, "Aby utworzyć grupę, musi się do niej zapisać co najmniej 3 graczy.")

	-- Create creation button
	self.createButton = self.contentFrame:Add("DButton")
	self:CreateButton(self.createButton, "Utwórz grupę", Color(111, 111, 136, 255), function()
		if string.utf8len(self.textEntry:GetValue()) <= 2 then
			LocalPlayer():NotifyLocalized("Nazwa grupy musi składać się z co najmniej 3 znaków!")
			return
		end
...
function PANEL:CreateNonActiveGroupPanel(group)
	...
	-- Create group title
	local title = self.contentFrame:Add("DLabel")
	self:CreateTitleLabel(title, group.name or "błąd")

	-- Create pending text
	local text = self.contentFrame:Add("DLabel")
	self:CreateLabel(text, "Grupa ta wciąż nie została utworzona i wymaga większej liczby członków.", Color(230, 230, 230, 255))

	-- Create text continuation to center properly
	local text2 = self.contentFrame:Add("DLabel")
	self:CreateLabel(text2, "Gdy grupa zostanie oficjalnie utworzona, uzyskasz dostęp do interfejsu grupy.", Color(230, 230, 230, 255), false, true)

	-- Create sign up player requirement text
	local warning = self.contentFrame:Add("DLabel")
	self:CreateWarningText(warning, "Aby utworzyć grupę, musi się do niej przyłączyć co najmniej 3 graczy.")

	-- Create acceptance number text
	local count = group:GetMembersCount() - 1
	self.label = self.contentFrame:Add("DLabel")
	self:CreateWarningText(self.label, count.."/3 graczy zaakceptowało", true)
...
function PANEL:CreateLeftSideButtons(group, role, character)
	...
	self:CreateLeftSideButton(infoButton, "Informacje", "willardnetworks/tabmenu/group/group_info.png", function()
	...
	self:CreateLeftSideButton(membersButton, "Członkowie", "willardnetworks/tabmenu/group/group_members.png", function()
		self:CreateRightSideTopBar("willardnetworks/tabmenu/group/group_members.png", "członkowie")
	...
	self:CreateLeftSideButton(forumsButton, "Forums", "willardnetworks/tabmenu/group/group_forums.png", function()
		self:CreateRightSideTopBar("willardnetworks/tabmenu/group/group_forums.png", "link do forum", function()
			self.groupUpdater = self:Add("ixGroupUpdater")
...
function PANEL:CreateRightSideMain(group, bNoTitle, bNoScrollPanel, bNoPadding)
	...
	if !bNoTitle then
		...
		title:SetText(group.name or "błąd")
...
function PANEL:CreateTabLabel(group, label)
	...
	if label == "" or label == nil then
		self.label:SetText("Nic tu jeszcze nie ma...")
	else
...
function PANEL:CreateRightSideTopBar(iconImage, name, editButtonCallback, role, permissions)
	...
	if editButtonCallback and hasAccess then
		...
		self.topbar.button:SetText("Edytuj "..name)
...
function PANEL:CreateMemberRow(memberData, role, group, character)
	...
	if (role and (role.id == GROUP_LEAD or role.id == GROUP_MOD)) then
		self.icon.OnMouseReleased = function(pnl, key)
			...
			if (memberData.role.id > role.id) then
				dmenu:AddOption("Wyrzuć", function()
					netstream.Start("ixGroupKick", group.id, memberData.id)
				end)
			end

			local subMenu = dmenu:AddSubMenu("Ustaw rolę")
	...
	name:SetText(memberData.name or "Nieznany")
	name:SizeToContents()
	name:DockMargin(0, SScaleMin(2 / 3), 0, 0)

	local description = "Nieznany"
	if !isstring(character) then
		description = localCharacter:DoesRecognize(character:GetID()) and memberData.description or "Nieznany"
	end
...
function PANEL:CreateAdminConfigRow(labelText, currentConfig, callback, bColor, group, bRole, roleID)
	...
	if callback then
		...
		rightSideButton:SetText(bRole and "Edytuj nazwę roli" or "Edytuj "..labelText)
...
function PANEL:CreateGroupAdminConfigs(role, group, character)
	if (role.id == GROUP_LEAD) then
		...
		self:CreateAdminConfigRow("nazwa grupy", group.name, function()
			Derma_StringRequest("Edytuj nazwę grupy", "Wprowadź nazwę grupy", group.name, function(text)
		...
		self.addRole:SetText("Dodaj rolę")
		...
		self.deleteGroup:SetText("Usuń grupę")
		...
		self.deleteGroup.DoClick = function(btn)
			surface.PlaySound("helix/ui/press.wav")
			Derma_Query("Czy na pewno chcesz usunąć tę grupę?", "Usunięcie grupy", "Tak", function()
		...
		self.hidden:SetText("Ukryj oddzielną kartę tabeli")
...
function PANEL:CreateLeaveButton()
	...
	self:CreateButton(self.leaveButton, "Wyjdź z grupy", Color(205, 114, 114, 255), function()
		netstream.Start("ixGroupLeave")
	end)
end
...
function PANEL:Init()
	...
	timer.Simple(0.25, function()
		...
		self:CreateLabel(title, "LargerTitlesFontFixed", color_white, string.utf8upper("zaproszenie do grupy"), true)
		...
		self:CreateLabel(whoInvited, "MenuFontFixed", Color(230, 230, 230, 255), "Zaproszony przez "..self.whoInvited)
		...
		self:CreateButton(accept, "Akceptuj", Color(96, 125, 68, 255), true, function()
			netstream.Start("ixGroupInvite", self.groupID)
			self:Remove()
		end)

		local decline = buttonPanel:Add("DButton")
		self:CreateButton(decline, "Odrzuć", Color(125, 68, 68, 255), false, function()
			self:Remove()
		end)
	end)
end
...
function PANEL:Init()
	...
	local title = self:Add("DLabel")
	title:SetFont("LargerTitlesFontNoClamp")
	title:SetText("Zaktualizuj treść")
	title:SizeToContents()
	...
	if parent.CreateTextEntry then
		parent:CreateTextEntry(self.textEntry, false, 0, true, true, true, "Napisz tutaj swoją zaktualizowaną treść...")
	end
