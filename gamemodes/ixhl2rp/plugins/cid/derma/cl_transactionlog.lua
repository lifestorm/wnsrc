...
function PANEL:Init()
	...
	title:Dock(LEFT)
	title:SetText("DZIENNIK TRANSAKCJI")
	...
	if (data and !table.IsEmpty(data)) then
		for _, tData in pairs(data) do
			...
			if (bShowReadButton) then
				...
				leftPanel.readUnRead:SetText(tonumber(tData.read) == 1 and "PRZECZYTANE" or "NIEPRZECZYTANE")
			...
			rightPanel.descPanel.amountLabel = self:CreateText(rightPanel.descPanel, "KWOTA: "..tData.amount.." ", RIGHT, 6, Color(169, 171, 27, 255))
			rightPanel.descPanel.amountLabel:DockMargin(0, 0, 0, SScaleMin(5 / 3))
			rightPanel.descPanel.reasonLabel = self:CreateText(rightPanel.descPanel, "POWÓD: ", LEFT, 5, Color(169, 171, 27, 255))
