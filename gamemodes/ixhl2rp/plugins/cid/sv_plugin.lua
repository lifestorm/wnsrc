...
ix.log.AddType("creditIDCardSetCredits", function(client, itemId, amount)
	return string.format("%s ustawił(a) kredyty karty #%d's na %d.", client:SteamName(), itemId, amount)
end)
...
ix.log.AddType("creditTransaction", function(client, senderCard, receiverCard, amount, posDevice, reason, senderCID, receiverCID)
	...
	if !senderCID then
		senderCardCid = senderCard:GetData("cid", "NIEPRAWIDŁOWY")
	else
		senderCardCid = (tostring(senderCard:GetData("cid", "00000")) == "00000" and senderCID or senderCard:GetData("cid", "NIEPRAWIDŁOWY"))
	end

	if !receiverCID then
		receiverCardCid = receiverCard:GetData("cid", "NIEPRAWIDŁOWY")
	else
		receiverCardCid = (tostring(receiverCard:GetData("cid", "00000") ) == "00000" and receiverCID or receiverCard:GetData("cid", "NIEPRAWIDŁOWY"))
	end

	local senderName = senderCard:GetData("name", "NIEZNANY")
	...
	local insert = mysql:Insert("ix_cid_transactions")
		...
		insert:Insert("receiver_name", receiverCard:GetData("name", "NIEZNANY"))
		...
		if (reason and reason != "") then
			...
			insert:Insert("reason", "nie podano powodu")
		end
	...
	return string.format("%s (CID: %s, przedmiot: %s) wysłał(a) %d kredytów do %s (CID: %s, przedmiot: %s) przez %s.",
		...
		receiverCardCid,
		receiverCard:GetID(),
		posDevice and string.format("Urządzenie POS dla '%s'", reason) or "terminal"
	)
end)
...
function PLUGIN:CreateIDCard(character, inventory, credits)
	...
	local data = {
		...
		geneticDesc = age.." | "..height.." | "..eyeColor.." OCZY",
		active = true,
