...
ix.command.Add("WithdrawCredit", {
	description = "Pobiera kredyty z aktywnej karty CID podanej postaci.",
	...
	OnRun = function(self, client, target, amount)
		...
		if (target:HasCredits(amount)) then
			...
			target:GetPlayer():Notify("Civil Authority Board pobrało "..amount.." kredytów z twojej karty.")
		else
			client:Notify("Postać nie posiada takiej liczby kredytów.")
		end
	end
})
		...
		OnRun = function(self, client, arguments)
			...
			if (IsValid(entity) and entity:IsDoor() and !entity:GetNetVar("disabled")) then
				...
				if (character:HasCredits(price)) then
					...
					return "@dPurchased", price..(price == 1 and " kredyt" or " kredytów")
				else
		...
		OnRun = function(self, client, arguments)
			...
			if (IsValid(entity) and entity:IsDoor() and !entity:GetNetVar("disabled")) then
				...
				if (client == entity:GetDTEntity(0)) then
					...
					return "@dSold", price..(price == 1 and " kredyt" or " kredytów")
				else
					-- Otherwise tell them they can not.
					return "@notOwner"
				end
