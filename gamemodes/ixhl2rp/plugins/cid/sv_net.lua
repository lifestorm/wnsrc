...
netstream.Hook("ixSendCredits", function(client, senderCardID, receiverCid, amount, senderCid)
	...
	local receiverQuery = mysql:Select("ix_characters")
		...
		--		return
			...
			ix.combineNotify:AddNotification("LOG:// Podmiot o ID #" .. senderCid .. " wysłał " .. amount .. " Kredytów do #" .. receiverCid, nil, client)
...
function PLUGIN.RequestSuccess(idCard, genericData, client, text, requestType)
	...
	local alertText, color = "%s, #%s (KS: %d) wnioskuje o pomoc", yellow

	if (client:IsCombine()) then
		alertText = "%s (KS: %d) wnioskuje o pomoc"
	elseif (isAC) then
		alertText, color = "Antyobywatel %s, #%s wnioskuje o pomoc", red
	elseif (isBOL) then
		alertText, color = "Poszukiwany BOL %s, #%s wnioskuje o pomoc", orange
	elseif (genericData.loyaltyStatus and genericData.loyaltyStatus != "BRAK") then
		if (genericData.loyaltyStatus == "Członek CAB") then
			alertText, color = "Członek CAB"..alertText, red
		else
			alertText, color = string.utf8sub(genericData.loyaltyStatus, 1, 6).." Konformista "..alertText, orange
		end
	elseif (genericData.socialCredits > 90) then
		if (genericData.socialCredits >= 175) then
			alertText = "Konformista Poziomu 3"..alertText
		elseif (genericData.socialCredits >= 125) then
			alertText = "Konformista Poziomu 2"..alertText
		else
			alertText = "Konformista Poziomu 1"..alertText
		end
	end

	if (idCard:GetData("active", false) == false) then
		alertText, color = alertText.." z nieaktywnym identyfikatorem", color != red and orange or red
	end
