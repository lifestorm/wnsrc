...
PLUGIN.energyConsumptionRateTooltips = {
	[1] = {
		maxRate = 0.002,
		color = "green",
		text = "Ten przedmiot jest lekki"
	},
	[2] = {
		maxRate = 0.004,
		color = "yellow",
		text = "Ten przedmiot jest średniej wagi"
	}
}
...
function PLUGIN:PopulateItemTooltip(tooltip, item)
	if (item.energyConsumptionRate) then
		...
		if (!color) then
			color, text = "red", "Ten przedmiot jest całkiem ciężki"
		end
...
function PLUGIN:AdjustInnerStatusPanel(innerStatus, CreateTitle, CreateSubBar)
	...
	if (!energyText) then
		energyText = "Wypoczęty"
	end

	CreateSubBar(energyPanel, "willardnetworks/hud/stamina.png", "Energia", energyText, smallerIconSize, smallerIconSize)
end
...
function PLUGIN:InitializedPlugins3()
	...
	function WNSkillPanel:CreateBoostInfo(boostPanel, skill)
		...
		if skillAttributes[2] then
			...
			boostedByLabel:SetText("Duże wzmocnienie od " .. skillAttributes[2].name)
		...
		if (varBoostLevel > 0 and boostedEnergy and boostedEnergyAmount and boostedEnergyAmount > 0) then
			...
			energyBoostedLevels:SetText("+" .. boostedEnergyAmount .. " poziomów od niskiego zmęczenia.")
		...
		if (varBoostLevel > 0) then
			...
			boostedLevels:SetText("+" .. varBoostLevel .. " poziomów od atrybutów postaci.")
		...
		if (varNeedsLevel > 0) then
			if (reducedEnergy) then
				...
				energyReducing:SetText("-" .. math.Round(reducedEnergy, 1) .. " poziomów od Zmęczenia")
			...
			if (reducedHunger) then
				...
				hungerReducing:SetText("-" .. math.Round(reducedHunger, 1) .. " poziomów od Głodu")
			...
			if (reducedThirst) then
				...
				thirstReducing:SetText("-" .. math.Round(reducedThirst, 1) .. " poziomów od Pragnienia")
			...
			if (reducedGas) then
				...
				gasReducing:SetText("-" .. math.Round(reducedGas, 1) .. " poziomów od Zarodników")
			...
			if (reducedHealth) then
				...
				healthReducing:SetText("-" .. math.Round(reducedHealth, 1) .. " poziomów od Obrażeń")
			...
			needsReducing:SetText("Łączna liczba obniżonych poziomów: -" .. varNeedsLevel)
