...
hook.Add("HUDPaint", "draw_bag_on_head", function()
    if (PLUGIN.LocalCharIsBagged) then
		...
		draw.SimpleText(
			"Na twojej głowie jest założony worek.",
...
net.Receive("WNDragOrBagPrompt", function(len)
    ...
    if (isBagged) then
        addButton("Usuń torbę", ResponseSelectRemoveBag)
    end

    if (isTied) then
        addButton("Rozwiąż", ResponseSelectUntie)
    end

    if (isFollowing) then
        addButton("Uwolnij", ResponseSelectRelease)
    else
        addButton("Ciągnij", ResponseSelectDrag)
    end
end)
