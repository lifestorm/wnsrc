...
ix.lang.AddTable("english", {
	dispOnlyChat = "Czat OOC dowódzctwa Dispatch, widzą go tylko inne Dispatch, skanery, i OTA.",
	dispAllCombine = "Nadaje ważny komunikat do wszystkich jednostek Kombinatu, w tym Civil Protection."
})
			...
			if (!speaker:IsDispatch() and !speaker:IsOTA() and speaker:Team() != FACTION_SERVERADMIN) then
				...
				if (delay > 0 and speaker.ixLastDispatchOOC) then
					...
					if (lastOOC <= delay and !CAMI.PlayerHasAccess(speaker, "Helix - Bypass OOC Timer", nil)) then
						speaker:Notify("Musisz poczekać jeszcze " .. delay - math.ceil(lastOOC) .. "s przed ponownym użyciem czatu OOC Dispatch!")
	...
	ix.command.Add("D", {
		...
		OnRun = function(self, client, message)
			if (ix.config.Get("allowDispatchOOC", true) or client:IsAdmin()) then
				...
				client:Notify("Czat OOC Dispatch jest obecnie wyłączony!")
			end
		end,
...
do
	...
	ix.chat.Register("scanner_radio", {
		...
		OnChatAdd = function(self, speaker, text)
			...
			local rText = string.format("[INFORMACJA] Dispatch: \"%s\"", text)
...
do
	...
	ix.chat.Register("dispatch_radio", {
		...
		OnChatAdd = function(self, speaker, text)
			local rText = string.format("[ROZKAZ] Dispatch: \"%s\"", text)
			local oldFont = self.font 
			local font = hook.Run("GetSpeakerFont", speaker)
...
do
	...
	ix.chat.Register("overwatch_radio", {
		...
		OnChatAdd = function(self, speaker, text)
			local rText = string.format("[ROZKAZ] Overwatch AI: \"%s\"", text)
			local oldFont = self.font 
			local font = hook.Run("GetSpeakerFont", speaker)
...
do
	...
	ix.chat.Register("dispatchcp_radio", {
		...
		OnChatAdd = function(self, speaker, text)
			local rText = string.format("[ROZKAZ] Dispatch do CP: \"%s\"", text)
			local oldFont = self.font 
			local font = hook.Run("GetSpeakerFont", speaker)
...
do
	...
	ix.chat.Register("dispatchota_radio", {
		...
		OnChatAdd = function(self, speaker, text)
			local rText = string.format("[ROZKAZ] Dispatch do OTA: \"%s\"", text)
			local oldFont = self.font 
			local font = hook.Run("GetSpeakerFont", speaker)
		...
		CanSay = function(self, speaker, text)
			if (!speaker:IsDispatch() and !speaker:IsOTA() and speaker:Team() != FACTION_SERVERADMIN) then
				...
				if (delay > 0 and speaker.ixLastCcgOOC) then
					...
					if (lastOOC <= delay and !CAMI.PlayerHasAccess(speaker, "Helix - Bypass OOC Timer", nil)) then
						speaker:Notify("Musisz poczekać " .. delay - math.ceil(lastOOC) .. "s przed ponownym użyciem czatu OOC CCG!")
	...
	ix.command.Add("CCG", {
		...
		OnRun = function(self, client, message)
			if (ix.config.Get("allowCcgOOC", true) or client:IsAdmin()) then
				...
				client:Notify("Czat OOC CCG jest obecnie wyłączony!")
			end
		end,
		OnCheckAccess = function(self, client)
			return client:IsDispatch() or client:IsOTA() or client:Team() == FACTION_SERVERADMIN
