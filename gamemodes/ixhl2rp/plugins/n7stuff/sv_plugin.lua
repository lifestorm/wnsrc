...
function PLUGIN:UpdateOldVortData(character, genericdata)
	...
	character:GetPlayer():Notify("Nowy format został pomyślnie zastosowany do pliku danych.")
end

function PLUGIN:CheckNotifyUpdate(author, character, data)
	if (character:IsVortigaunt()) then
		if (data.cid == "N/A") then
			ix.combineNotify:AddNotification("LOG:// " .. author .. " przeprowadza inspekcję tożsamości na " .. string.upper("zasobie biotycznym o obroży:") .. " #" .. character:GetCollarID())
		else
			ix.combineNotify:AddNotification("LOG:// " .. author .. " przeprowadza inspekcję tożsamości na " .. string.upper("zasobie biotycznym o identyfikatorze:") .. " #" .. data.cid)
		end
	else
		ix.combineNotify:AddNotification("LOG:// " .. author .. " przeprowadza inspekcję tożsamości na '" .. string.upper(data.name) .. "', #" .. data.cid)
	end
end
	...
	local query = mysql:Select("ix_characters")
		...
		query:Callback(function(result)
			...
			dataSelect:Callback(function(dataSelectResult)
				...
				SendDatabaseAnswer(client, util.JSONToTable(dataSelectResult.data), { result.name .." | #"..result.cid, "Sukces" })
			end)
			dataSelect:Execute()
		end)
	query:Execute()
