...
if SERVER then
	...
	function ENT:Use(user)
		...
		if self:GetNetVar("destroyed", false) == true and !item then
			user:Notify("Do naprawienia szkody potrzebny jest zestaw narzędzi") -- message when dont work and you dont have item
			return false
		else
			if item and self:GetNetVar("destroyed", false) == true then
				user:Freeze(true)
				user:SetAction("Naprawianie...", 12, function() -- line with text when you repair it
					...
					if (item) then
						...
						self:Remove()
						user:Notify("Pomyślnie naprawiono uszkodzenie elektryczne") -- message when you finish the job
					end
				end)
			end
		end
