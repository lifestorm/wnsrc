...
if SERVER then
	...
	function ENT:Use(user)
		...
		if self:GetNetVar("destroyed", false) == true and !item then
			user:Notify("Doszło do wycieku wody! Do naprawy potrzebny jest zestaw narzędzi.") -- message when dont work and you dont have item
			return false
		else
			if item and self:GetNetVar("destroyed", false) == true then
				...
				user:SetAction("Naprawianie..", 12, function() -- line with text when you repair it
					...
					if (item) then
						...
						self:Remove()
						user:Notify("Rurociąg został naprawiony.") -- message when you finish the job
					end
				end)
			end
		end
