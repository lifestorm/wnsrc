...
function ENT:OnRemove()
	...
	function ENT:Use(user)
		...
		if self:GetNetVar("destroyed", false) == true and !item then
			user:Notify("Rura jest uszkodzona! Do jej naprawy potrzebny jest zestaw narzędzi.") -- message when dont work and you dont have item
			return false
		else
			if item and self:GetNetVar("destroyed", false) == true then
				...
				user:SetAction("Naprawianie...", 12, function() -- line with text when you repair it
					...
					if (item) then
						...
						self:Remove()
						user:Notify("Naprawiłeś rurę.") -- message when you finish the job
					end
				end)
			end
		end
