...
if (CLIENT) then
	...
	function ITEM:PopulateTooltip(tooltip)
		...
		panel:SetText("Nazwa: " .. self:GetData("ownerName", "NIEZNANY"))
		panel:SizeToContents()
	end
end
...
function ITEM:OnEquipped(client)
    ...
    elseif (self:GetData("trackingActive")) then
        ix.combineNotify:AddImportantNotification("WRN:// Wykryto niezgodność biosygnału z uniformem dla " .. self:GetData("ownerName"), nil, client, client:GetPos(), nil)
    end
...
function ITEM:OnInstanced()
	...
	self:SetData("ownerName", "NIEZNANY")
...
ITEM.functions.Repair = {
	...
	OnRun = function(item)
		...
		player:Freeze(true)
        player:SetAction("Naprawianie Pancerza...", 5, function()
	...
	OnCanRun = function(item)
		...
		if timer.Exists("combattimer" .. item.player:SteamID64()) then
			item.player:Notify("Nie można używać tego przedmiotu podczas walki.")
			return false
		end
...
ITEM.functions.AdmActivateDe = {
	name = "(ADM) Dezaktywuj kombinezon",
...
ITEM.functions.Pants = {
	...
	OnRun = function(item)
		...
		if (item.bodyGroups.Pants != actual) then
			...
			item.player:Notify("Ustawiono normalne buty.")
		else
			item.bodyGroups.Pants = 1
			item:SetData("Pants", 1)
			item.player:Notify("Ustawiono bojowe buty.")
		end
		return false
	end,
	OnCanRun = function(item)
