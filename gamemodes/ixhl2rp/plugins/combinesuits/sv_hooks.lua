...
function PLUGIN:OnPlayerCombineSuitChange(client, bEquipped, bActive, suit, bLoadout)
	...
	if (bEquipped) then
		...
		if (!bLoadout or (client.ixCharLoadedCPSuit and client.ixCharLoadedCPSuit + 5 < CurTime())) then
			...
			ix.combineNotify:AddNotification("NTC:// Jednostka " .. client:GetCombineTag() .. " online", Color(255, 100, 255, 255), suit:GetData("trackingActive") and client, nil, nil)
		end
	else
		client:SetNetVar("combineSuitType", 0)
		client:SetNetVar("combineSuitName", nil)
		...
		if (suit:GetData("trackingActive") and (!client:IsInArea() or !ix.area.stored[area].properties.nexus)) then
			ix.combineNotify:AddImportantNotification("WRN:// Anonimowość jednostki " .. suit:GetData("ownerName") .. " naruszona", nil, suit:GetData("trackingActive") and client, client:GetPos(), nil)
		else
			ix.combineNotify:AddNotification("NTC:// Jednostka " .. suit:GetData("ownerName") .. " offline", Color(255, 100, 255, 255), suit:GetData("trackingActive") and client, nil, nil)
		end
	end
end
...
function PLUGIN:OnPlayerAreaChanged(client, newArea, id)
	...
	if (client:GetNetVar("ProtectionTeamOwner")) then
		ix.combineNotify:AddNotification("LOG:// Relokacja Protection-Team " .. client:GetNetVar("ProtectionTeam") .. " do " .. id)
	elseif (!client:GetNetVar("ProtectionTeam")) then
		ix.combineNotify:AddNotification("LOG:// Relokacja jednostki " .. client:GetCombineTag() .. " do " .. id)
		cooldown = 30
	end
