...
function PANEL:Init()
	self.scrollingmessage = "SOCIO-STABILNOŚĆ NIENARUSZONA" -- Default Message
...
function PANEL:Close()
	...
	self:SizeTo(-1, 0, 0.3, 0, -1, function()
		...
		if (self.shouldReopen) then
			...
			self.scrollingmessage = GetNetVar("visorText", "SOCIO-STABILNOŚĆ NIENARUSZONA")
			self:Open()
		end
	end) -- Animation
	self:MoveTo(x, (y + (textH / 2)), 0.3, 0)
...
function PANEL:Think()
	...
	if (LocalPlayer():HasActiveCombineMask() or LocalPlayer():IsDispatch()) then
		...
		if (self.scrollingcolor != GetNetVar("visorColor", "blue") or self.scrollingmessage != GetNetVar("visorText", "SOCIO-STABILNOŚĆ NIENARUSZONA")) then
