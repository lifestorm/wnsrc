...
ITEM.width = 1
ITEM.height = 1
ITEM.description = "Po jej użyciu możesz ustawić hasło na pojemnik lub drzwi."
ITEM.category = "Tools"
ITEM.colorAppendix = {["blue"] = "Kłódka działa również na drzwi.", ["red"] = "Kłódka może być odstrzelona tylko jeżeli jej właściciel jest online."}
...
ITEM.functions.Lock = {
	...
	OnRun = function(itemTable)
		...
		if (!IsValid(trace.Entity) or !IsValid(trace.Entity:GetPhysicsObject())) then
			client:NotifyLocalized("Nie możesz tego użyć na tych drzwiach/pojemniku!")
			return false
		end
		...
		if (!table.HasValue(allowedModels, model)) then
			client:NotifyLocalized("To nie jest poprawny model!")
			return false
		end

		if (client.ixPlacingLock and client.ixPlacingLock > CurTime()) then
			client:NotifyLocalized("Poczekaj chwilę zanim użyjesz tego przedmiotu!")
			return false
		end
		...
		if (traceClass == "ix_wncontainer") then
			...
			if (entity:GetType() != entity.PUBLIC and entity:GetType() != entity.GROUP and
				(entity:GetType() != entity.PRIVATE or entity.ownerCharID != client:GetCharacter():GetID())) then
					client:NotifyLocalized("Nie możesz tego użyć na tym pojemniku!")
					return false
			else
