...
ix.lang.AddTable("english", {
	optVortSensingDisable = "Wyłącz vortalne wykrycie",
	optdVortSensingDisable = "Czy vortalne wykrycie powinno być wyłączone.",
})
...
ix.command.Add("ToggleVortalSensing", {
	description = "Przełącz vortalne wykrycie.",
...
ix.command.Add("VortSetConterminous", {
	...
	description = "Ustawia Vortigaunt jako konterminalnego.",
	adminOnly = true,
    arguments = {
        ix.type.character,
		ix.type.bool
...
ix.command.Add("VortSetAgenda", {
	description = "Ustala agendę dla konterminalych Vortigauntów.",
	adminOnly = false,
    arguments = {
        bit.bor(ix.type.text, ix.type.optional)
    },
	OnRun = function(self, client, message)
		local textDefault = "Vortigaunt milczy."
...
ix.command.Add("VortCheckAgenda", {
	description = "Sprawa agendę konterminalnych Vortigauntów.",
	adminOnly = false,
	OnRun = function(self, client, message)
...
ix.command.Add("rvort", {
	description = "Przekazywanie ważnych informacji innym konterminalnym Vortigauntom.",
	adminOnly = false,
    arguments = {
        ix.type.text
    },
...
ix.command.Add("thevortigaunt", {
	description = "Przemawiaj do wszystkich połączonych vortów jako głos Vortigaunta.",
	adminOnly = false,
    arguments = {
        ix.type.text
    },
...
ix.command.Add("ToggleVortalVision", {
	description = "Przełącz vortalną wizję.",
	OnRun = function(self, client, arguments)
...
ix.command.Add("VortStun", {
	description = "Ogłusz osobę stojącą przed tobą.",
	OnRun = function(self, client)
		local percentage = 10 / 100
		if !client:HasVortalEnergy(10 + (percentage * client:Armor())) then
			return client:NotifyLocalized("Nie masz wystarczającej ilości energii vortalnej.")
		end
		...
		if !IsValid(tr.Entity) or !tr.Entity:IsPlayer() or tr.Entity.ixRagdoll or !tr.Entity:Alive() then
			return client:NotifyLocalized("Nieprawidłowy cel!")
		end
		if tr.Entity:IsVortigaunt() and tr.Entity:GetNetVar("ixVortMeditation") then
			return client:NotifyLocalized("Nie można ogłuszyć medytującego vorta!")
		end
		if client:GetNetVar("ixVortMeditation") then
			return client:NotifyLocalized("Nie możesz ogłuszać podczas medytacji!")
		end
...
ix.command.Add("VortUnlock", {
	description = "Odblokowuje czyjeś kajdany.",
	OnRun = function(self, client)
		local forward = client:EyeAngles():Forward()
		local tr = util.QuickTrace(client:EyePos(), forward * 128, client)
		...
		timer.Simple(2.25, function()
			...
			if foundSomething then
				ix.combineNotify:AddImportantNotification(collarID and "WRN:// Kajdany Vortigaunta otrzymały krytyczne obrażenia. ID obroży: #" .. collarID or "WRN:// Kajdany Vortigaunt otrzymały krytyczne obrażenia. ID obroży: NIEZNANY.", Color(255, 81, 0), tr.Entity, tr.Entity:GetPos())
				tr.Entity:NotifyLocalized("Kajdany nie są już zablokowane i możesz je zdjąć.")
			else
				tr.Entity:NotifyLocalized("Nie masz na sobie żadnych zamkniętych kajdan...")
				return
			end
			client:GetCharacter():DoAction("vort_beam_practice")
		end)
...
ix.command.Add("EnterMeditation", {
	description = "Przełącz medytację vortalną i odzyskaj swoją vortalną energię.",
	OnRun = function(self, client)
		if client:GetNetVar("ixVortMeditation") then
...
ix.command.Add("SetVortAllowResetCollar", {
	...
	adminOnly = true,
	OnRun = function(self, client, character, bool)
		if (character:GetFaction() == FACTION_VORT and character:GetBackground() != "Biotyk" and character:GetBackground() != "Kolaborator") then
			if (bool) then
...
ix.command.Add("vortessence", {
	description = "Otwiera menu vortessencji jako Vortigaunt.",
	OnCheckAccess = function(self, client)
