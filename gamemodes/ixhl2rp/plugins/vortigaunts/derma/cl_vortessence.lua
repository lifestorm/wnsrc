...
function PANEL:CreateTopTitle(parent)
	...
	titleText:Dock(LEFT)
	titleText:SetText("Vortessencja")
...
function PANEL:CreateButtons(parent)
	...
	searchBar:SetPlaceholderText("Wyszukaj tytuł notatki...")
	...
	for i = 1, 3 do
		...
		info:SetText(charInfoButton and "Notatki Postaci" or eventButton and "Notatki Wydarzeń" or "Notatki Ogólne")
...
function PANEL:CreateContent(parent, info)
	...
	self.content.name = info == 1 and "Notatki Postaci" or info == 2 and "Notatki Wydarzeńi" or "Notatki Ogólne"
...
function PANEL:CreateAdd(parent, category)
	...
	add.DoClick = function()
		surface.PlaySound("helix/ui/press.wav")
		Derma_StringRequest(
			"Dodaj Notatkę | "..parent.name,
			"Ustaw tytuł twojej notatki:",
			"",
			function(title)
				if string.len(title) <= 0 then
					LocalPlayer():NotifyLocalized("Nie można wprowadzić pustego tytułu!")
					return false
				end
