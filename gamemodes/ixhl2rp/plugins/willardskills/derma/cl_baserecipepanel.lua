...
function PANEL:CreateRecipeCategories(query)
	...
	for k, v in ipairs(categories) do
		...
		if (v.category == "Poziom odblokowywuje") then
...
function PANEL:Init()
	...
	if parent.skill != "guns" and parent.skill != "speed" and parent.skill != "melee" then
		...
		search:SetPlaceholderText("Szukaj czegoś...")
	...
	parent.craftingAttributeFrame.title:SetText("Wzmocnienia:")
	...
	parent.craftingAttributeText:SetText("Ten przedmiot nie daje żadnych wzmocnień atrybutów")
	...
	or self:GetParent().skill == "chemistry" or self:GetParent().skill == "medicine" then
		...
		parent.stationsTitle:SetText(parent.stationsTitleText or "Stacje: ")
		...
		parent.toolsTitle:SetText(parent.toolsTitleText or "Narzędzia: ")
		...
		parent.ingredientsTitle:SetText(parent.ingredientsTitleText or "Składniki:")
	...
	if (parent.recipeData.recipe.cost) then
		...
		if (parent.recipeData.recipe.skill == "bartering") then
			...
			parent.stockTitle:SetText(parent.stockTitleText or "Zapasy: ")
			...
			parent.licenseTitle:SetText(parent.licenseTitleText or "Zezwolenie: ")
			...
			parent.amountTitle:SetText(parent.amountTitleText or "Ilość zakupu: ")
		...
		if (parent.recipeData.recipe.skill == "bartering" or parent.recipeData.recipe.skill == "smuggling") then
			...
			if (parent.recipeData.recipe.skill == "smuggling") then
				parent.infoText:SetText("Musisz znaleźć przemytnika w kanałach, aby kupić i/lub sprzedać ten przedmiot.")
			else
				parent.infoText:SetText("Zakupione przedmioty mogą zostać odebrane z terminali znajdujących się na ulicach.")
			end
...
function PANEL:Init()
	...
	if (self.recipe.skill != "bartering") then
		...
		if self.recipe.level then
			if (self.recipe.level > character:GetSkillLevel(self.recipe.skill)) and !self.recipe.noIngredients then
				...
				self.levelRequirement:SetText("Wymaga poziomu ".. self.recipe.level)
		...
		if (!character:HasPermit(self.recipe.category)) then
			...
			self.permitRequirement:SetText("Wymaga zezw. "..self.recipe.category)
	...
	if self.recipe.noIngredients then
		...
		if self.recipe.skillScale then
			...
			self.levelRequirement:SetText("Wymaga poziomu ".. self.recipe.level or "")
		end
...
function PANEL:RebuildCrafting()
	...
	if self.recipe.cost then
		...
		if (self.recipe.skill == "smuggling") then
			costText:SetText("Kupno: ")
		else
			costText:SetText("Koszt: ")
		end
		...
		if self.recipe.skill == "bartering" then
			...
			parentFrame.actualStock:SetText("Dostępne: "..barteringStock)
	...
	if (parentFrame.craftButton) then
		...
		if self.recipe.skill == "bartering" then
			parentFrame.craftButton:SetText("Zakup "..self.recipe.name)
		else
			parentFrame.craftButton:SetText("Stwórz "..self.recipe.name)
		end
			...
			parentFrame.craftButton.DoClick = function()
				...
				if self.recipe.skill == "bartering" then
					...
					if table.Count(character:GetPurchasedItems()) == ix.config.Get("PickupDispenserMaxItems") then
						LocalPlayer():NotifyLocalized("Nie możesz kupić tego przedmiotu, ponieważ masz zbyt wiele przedmiotów do odbioru.")
						return
					end

					if !character:HasPermit(self.recipe.category) then
						LocalPlayer():NotifyLocalized("Nie masz zezwolenia!")
						return
					end
					...
					if itemID then
						if ix.item.instances[itemID] then
							...
							if (credits < self.recipe.cost) then
								LocalPlayer():NotifyLocalized("Nie masz wystarczająco dużo pieniędzy!")
								return false
							end

							if barteringStock == 0 then
								LocalPlayer():NotifyLocalized("Nie można kupić tego produktu z powodu niskiego stanu magazynowego")
								return
							end

							timer.Simple(0.1, function()
								parentFrame.actualStock:SetText(ix.city.main.items[recipeItem] and ix.city.main.items[recipeItem].amount .. " Dostępnych" or 0 .. " Dostępnych")
							end)
						else
							LocalPlayer():NotifyLocalized("Nie udało się znaleźć Twojej karty ID!")
							return false
						end
					else
						LocalPlayer():NotifyLocalized("Nie masz aktywnej karty ID!")
						return false
					end
				end
	...
	if (result and result > 0) then
		local text = "Zwiększa Przemyt o "..result.." xp"
		if (self.recipe.skill == "smuggling") then
			if (self.recipe.cost != "N/A") then
				text = "Kupno zwiększa Przemyt o "..result.." xp"
			else
				text = "Sprzedaż zwiększa Przemyt o "..result.." xp"
			end
		end
	...
	elseif self.recipe.level and (self.recipe.level > character:GetSkillLevel(self.recipe.skill)) then
		parentFrame.itemLevelUp:SetText("Ten przepis wymaga poziomu "..self.recipe.level)
		parentFrame.itemLevelUp:SetTextColor(Color(191, 66, 67, 255))
	else
		parentFrame.itemLevelUp:SetText("Ten przepis nie zwiększy Przemytu")
		parentFrame.itemLevelUp:SetTextColor(Color(191, 66, 67, 255))
	end
	...
	if (parentFrame.itemLevelUpBuy) then
		if (bHasExp and self.recipe.cost != "N/A" and self.recipe.buyPrice != "N/A") then
			...
			if (result and result > 0) then
				parentFrame.itemLevelUpBuy:SetText("Sprzedaż zwiększa Przemyt o "..result.." xp")
				if (result >= 5) then
			...
			elseif self.recipe.level and (self.recipe.level > character:GetSkillLevel(self.recipe.skill)) then
				parentFrame.itemLevelUpBuy:SetText("Ten przepis wymaga poziomu "..self.recipe.level)
				parentFrame.itemLevelUpBuy:SetTextColor(Color(191, 66, 67, 255))
			else
				parentFrame.itemLevelUpBuy:SetText("Ten przepis nie zwiększy "..self.recipe.skill)
				parentFrame.itemLevelUpBuy:SetTextColor(Color(191, 66, 67, 255))
			end
	...
	local index = ix.item.list[k] or LocalPlayer():NotifyLocalized("NIEPRAWIDŁOWY WYNIK")
	...
	if istable(index) then
		if !index.boosts then
			...
			if index.boosts.strength then
				desc = desc.."Siła "..index.boosts.strength.." | "
			end

			if index.boosts.agility then
				desc = desc.."Zręczność "..index.boosts.agility.." | "
			end


			if index.boosts.intelligence then
				desc = desc.."Inteligencja "..index.boosts.intelligence.." | "
			end


			if index.boosts.perception then
				desc = desc.."Percepcja "..index.boosts.perception.." | "
			end
	...
	local licenseName = "Zezwolenie Działalności"
	local toolName = "Nie jest wymagane narzędzie"
	local stationName = "Nie jest wymagana stacja"
	...
	if self.recipe.tools and istable(self.recipe.tools) then
		for _, v in pairs(self.recipe.tools) do
			if toolName == "Nie jest wymagane narzędzie" then
	...
	if self.recipe.station then
		if !istable(self.recipe.station) then
			...
			for _, v in pairs(self.recipe.station) do
				if stationName == "Nie jest wymagana stacja" then
			...
			if self.recipe.canUseCampfire == true then
				stationName = stationName.." lub ognisko"
			end
		end
	end
