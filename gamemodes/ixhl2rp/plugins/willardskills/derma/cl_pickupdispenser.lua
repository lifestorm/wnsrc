...
function PANEL:CreateTopBar()
	...
	self.titleText:SetText("Wybierz element do wydania")
...
function PANEL:CreateItems()
	...
	for k, v in pairs (LocalPlayer().boughtItems) do
		...
		local actualTitle = istable(v) and v.title or ix.item.list[k].name or "Pudełko kartonowe"
	...
	invisibleButton.DoClick = function()
		if inventory then
			if inventory:FindEmptySlot(width, height) then
				...
				LocalPlayer():NotifyLocalized("Odebrałeś "..actualTitle)
				surface.PlaySound("helix/ui/press.wav")
				netstream.Start("SetPurchasedItems", k, LocalPlayer().activePickupDispenser)
			else
				LocalPlayer():NotifyLocalized("Nie masz wystarczająco dużo miejsca dla "..actualTitle.."!")
			end
		end
	end
...
function PANEL:CreateItem(scrollPanel, itemID, number)
	...
	local actualTitle = istable(number) and number.title or ix.item.list[itemID].name or "Pudełko kartonowe"
	title:Dock(TOP)
