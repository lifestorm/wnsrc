...
function PANEL:Init()
	...
	skillsTitle:SetText("Poziomy postaci " .. LocalPlayer():Name())
	...
	skillsDesc:SetText("Maksymalny łączny poziom umiejętności to " .. ix.config.Get("MaxTotalSkill", 0) .. ".")
	...
	skillsDesc2:SetText("Rozdane punkty umiejętności: " .. LocalPlayer():GetCharacter():GetTotalSkillLevel() .. "/" .. ix.config.Get("MaxTotalSkill", 0))
...
function PANEL:CreateSkills()
	...
	for skillID, skillInfo in SortedPairs(ix.skill.list) do
		if character:GetFaction() != FACTION_VORT and skillInfo.uniqueID == "vort" then
			continue
		end
...
function PANEL:CreateSkillPanel(skillID, skillInfo)
	...
	self:CreateSkillLabel(skillPanel.skillLevel, "Poziom: " .. character:GetSkillLevel(skillID) .. "/" .. ix.skill.MAX_SKILL, "MenuFontLargerNoClamp", 0, 0, Color(255, 204, 0, 255))
	...
	if skillInfo.uniqueID == "bartering" then
		experiencePanel:SetVisible(false)
		skillPanel.skillLevel:SetText("Masz " .. (istable(character:GetPermits()) and table.Count(character:GetPermits()) or "0") .. " aktywnych zezwoleń")
...
function PANEL:CreateBoostInfo(boostPanel, skill)
	...
	if skillAttributes[2] then
		...
		boostedByLabel:SetText("Znaczne wzmocnienie od " .. skillAttributes[2].name)
	...
	if skillAttributes[1] then
		local boostedByLabel = boostPanel:Add("DLabel")
		boostedByLabel:SetText("Niewielkie wzmocnienie od " .. skillAttributes[1].name)
		boostedByLabel:SetFont("MenuFontBoldNoClamp")
		boostedByLabel:SetContentAlignment(4)
		boostedByLabel:SizeToContents()
		boostedByLabel:Dock(TOP)
	...
	if (varBoostLevel > 0) then
		...
		boostedLevels:SetText("Wzmocnione poziomy: +" .. varBoostLevel)
	...
	if (varNeedsLevel > 0) then
		if (reducedHunger) then
			...
			hungerReducing:SetText("-" .. math.Round(reducedHunger, 1) .. " poziomy z powodu Głodu")
		...
		if (reducedThirst) then
			...
			thirstReducing:SetText("-" .. math.Round(reducedThirst, 1) .. " poziomów z powodu Pragnienia")
		...
		if (reducedGas) then
			...
			gasReducing:SetText("-" .. math.Round(reducedGas, 1) .. " poziomu z powodu Zarodników")
		...
		if (reducedHealth) then
			...
			healthReducing:SetText("-" .. math.Round(reducedHealth, 1) .. " poziomu z powodu Urazów")
		...
		needsReducing:SetText("Łączne obniżenie poziomu: -" .. varNeedsLevel)
...
function PANEL:CreateNewContent(v, name)
	...
	self.backButton:SetText("Powrót do menu umiejętności")
...
function PANEL:CreateAttributesPanel()
	...
	self.attributesButton:SetText("Wzmocnienia Atrybutów")
...
function PANEL:CreateAttributeBar(parent, attributeID)
	...
	if specialBoost.long then
		...
		longBoost:Dock(TOP)
		if (level != 0) then
			local change = level > specialBoost.long.target and "zmniejszy się" or "zwiększy się"
			local sign = level > 0 and "+" or ""
			longBoost:SetText(string.format("Obecnie masz %s%d wzmocnienie od żywności, %s o 1 za %d min", sign, level, change, specialBoost.long.time))
		elseif (specialBoost.long.time) then
			local sign = specialBoost.long.target > 0 and "+" or "-"
			longBoost:SetText(string.format("%s1 wzmocnienie od żywności stanie się aktywne za %d min", sign, specialBoost.long.time))
		end
	...
	if specialBoost.short then
		...
		shortBoost:Dock(TOP)
		if (level != 0) then
			local change = level > specialBoost.short.target and "zmniejszy się" or "zwiększy się"
			local sign = level > 0 and "+" or ""
			shortBoost:SetText(string.format("Obecnie masz %s%d wzmocnienie od substancji, %s o 1 za %d s", sign, level, change, specialBoost.short.time))
		elseif (specialBoost.short.time) then
			local sign = specialBoost.short.target > 0 and "+" or "-"
			shortBoost:SetText(string.format("%s1 wzmocnienie od substancji stanie się aktywne za %d s", sign, specialBoost.short.time))
		end
	...
	if specialBoost.neg then
		...
		negativeBoost:SetText("-" .. specialBoost.neg.level .. " od głodu i pragnienia")
...
function PANEL:CreateAttributePopup()
	...
	titleText:Dock(LEFT)
	titleText:SetText("Wzmocnienia Atrybutów")
