...
function PANEL:Init()
	...
	titleText:Dock(LEFT)
	titleText:SetText("Administracja Cywilna")
...
function PANEL:CreateNewspaperButton()
	...
	self.newspapers:SetText("Gazety")
	...
	self.newspapers.DoClick = function()
		...
		for k, v in pairs(self.storedNewspapers) do
			...
			newspaper.DoClick = function()
				...
				label:SetText("Usunąć przechowywane gazety?")
...
function PANEL:CreateDoorAccessButton()
	...
	self.doorAccess:SetText("Drzwi Kombinatu")
...
function PANEL:CreateInner()
	...
	self.barteringConfigs:SetText("Handel")
		...
		for uniqueID, RECIPE in SortedPairs(recipes) do
			if RECIPE.cost and RECIPE.skill == "bartering" then
				if !table.HasValue(categories, RECIPE.category) then
					...
					priceMultiplier:SetText("Mnożnik ceny "..RECIPE.category)
					...
					priceMultiplier.DoClick = function()
						surface.PlaySound("helix/ui/press.wav")
						Derma_StringRequest(
							"Ustaw mnożnik "..RECIPE.category,
							"Mnożnik ceny "..RECIPE.category.." (maks. 1 miejsce po przecinku, domyślnie 1,0)",
							...
							function(number)
								...
								if isnumber(toNumber) then
									...
									if (toNumber <= maxValue) and (toNumber >= minValue) then
										...
										LocalPlayer():NotifyLocalized("Ustawiono mnożnik ceny handlu "..RECIPE.category.." na "..number)
									else
										LocalPlayer():NotifyLocalized("Liczba musi być mniejsza lub równa (⩽) 100 i większa lub równa (⩾) 0.")
									end
								end
							end,
	...
	self.ration.DoClick = function()
		...
		rationInterval:SetText("Interwał Racji")
		...
		rationInterval.DoClick = function()
			surface.PlaySound("helix/ui/press.wav")
			Derma_StringRequest(
				"Ustaw Interwał Racji",
				"Ustaw Interwał Racji w godzinach (0 miejsc po przecinku, domyślnie 4)",
				...
				function(number)
					...
					if isnumber(toNumber) then
						...
						if (toNumber <= maxValue) and (toNumber >= minValue) then
							...
							LocalPlayer():NotifyLocalized("Ustawiono licznik Interwału Racji na "..number.. " h")
						else
							LocalPlayer():NotifyLocalized("Liczba musi być mniejsza lub równa (⩽) 10 i większa lub równa (⩾) 1.")
						end
					end
				end,
			nil)
	...
	if istable(LocalPlayer().messagelist) then
		if !table.IsEmpty(LocalPlayer().messagelist) then
			...
			self.messages:SetText("Wiadomości")
			...
			self.messages.DoClick = function()
				...
				for k, v in pairs(LocalPlayer().messagelist) do
					...
					message.DoClick = function()
						...
						if v["message_reply"] then
							sendReply:SetText("PRZECZYTAJ ODPOWIEDŹ")
						else
							sendReply:SetText("ODPOWIEDŹ")
						end
						...
						sendReply.DoClick = function()
							...
							sendReply:SetText("ODPOWIEDZ")
							textEntry:SetValue("Napisz tutaj swoją odpowiedź..")
