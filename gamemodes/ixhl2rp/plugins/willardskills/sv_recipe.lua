...
netstream.Hook("CraftRecipe", function(client, data, barteringStock)
	...
	if (!bPlayerCanCraft) then
		if (!err) then
			err = "Nie możesz teraz wytwarzać (ale Dev spierdolił i zapomniał dodać dlaczego)!"
		end
	...
	if (!bCanCraftRecipe) then
		if (!err2) then
			err2 = "Nie można tego wytworzyć (ale Dev spierdolił i zapomniał dodać dlaczego)!"
		end
	...
	if !barteringStock then
		...
		client:NotifyLocalized("Pomyślnie utworzono "..recipe.name)
	else
		...
		local k = next(result)
		local index = ix.item.list[k] or client:NotifyLocalized("NIEPRAWIDŁOWY WYNIK")
		if istable(index) then
			local resultName = index.name or "NIEPRAWIDŁOWY"
			...
			client:NotifyLocalized("Pomyślnie kupiono "..resultName)
...
function PLUGIN:PlayerCanCraft(client)
	...
	if (client.ixNextCraftTime and curTime < client.ixNextCraftTime) then
		return false, "Musisz poczekać "..tostring(math.Round(client.ixNextCraftTime - curTime, 1)).." sekund."
	end
