...
function PANEL:Init()
	local informedCitizen = function()
		...
		self.headlineText = self:CreateTextEntry(self.mainPanel, SScaleMin(164 / 3), SScaleMin(64 / 3), TOP, 0, 0, 0, 0, nil, "UnifrakturTitle", 20, true, true, "", nil, "Świadomy Obywatel")
		...
		self.bigHeadline = self:CreateTextEntry(self.mainPanel, nil, SScaleMin(117 / 3), TOP, SScaleMin(33 / 3), SScaleMin(3 / 3), SScaleMin(33 / 3), 0, nil, "FjallaOneRegularLarge", 20, false, true, "", nil, string.upper("TERROR NA ULICACH"))
		self.bigHeadline:SetZPos(1)
		self.subHeadline = self:CreateTextEntry(self.mainPanel, nil, SScaleMin(60 / 3), TOP, SScaleMin(33 / 3), 0, SScaleMin(33 / 3), 0, nil, "FjallaOneRegularSmall", 52, false, true, "", nil, string.upper("MIESIĘCZNA BLOKADA OGŁOSZONA PO WYBUCHU ZAMIESZEK"))
		...
		self.imageButton = self:CreateSelectorButton(left, SScaleMin(534 / 3), SScaleMin(480 / 3), TOP, 0, 0, 0, 0, nil, "WYBIERZ OBRAZ", function()
			-- if self.cracked then LocalPlayer():Notify("You cannot add images to cracked newspapers!") return end

			Derma_StringRequest(
				"URL (wszelkie zdjęcia o treści erotycznej spowodują zbanowanie)",
				"Wprowadź adres URL obrazu, który chcesz wyświetlić.",
				"",
				function(url)
		...
		self.articleTitle = self:CreateTextEntry(left, nil, SScaleMin(60 / 3), TOP, 0, 0, 0, 0, nil, "AdobeCaslonProBold", 30, false, true, "", nil, "Admin C24. Stewart rezygnuje")
		...
		self.articleText1 = self:CreateTextEntry(articleContentPanel, (SScaleMin(534 / 3) / 2) + SScaleMin(3 / 3), nil, LEFT, 0, 0, 0, 0, nil, "AbrilTitlingRegular", 655, true, true, "", nil, "Tekst artykułu powinien być tutaj")
		self.articleText1:SetZPos(7)
		self.articleText1.bAlignTopLeft = true
		self.articleText1:DockMargin(-SScaleMin(3 / 3), 0, 0, 0)

		self.articleText2 = self:CreateTextEntry(articleContentPanel, (SScaleMin(534 / 3) / 2) - padding / 2, nil, RIGHT, 0, 0, 0, 0, nil, "AbrilTitlingRegular", 655, true, true, "", nil, "Część 2 Tekstu artykułu powinna być tutaj")
		self.articleText2:SetZPos(8)
		self.articleText2.bAlignTopLeft = true

		self.imageButton2 = self:CreateSelectorButton(right, SScaleMin(283 / 3) - SScaleMin(13 / 3), SScaleMin(202 / 3), TOP, 0, 0, 0, 0, nil, "WYBIERZ OBRAZ", function()
			-- if self.cracked then LocalPlayer():Notify("You cannot add images to cracked newspapers!") return end
			Derma_StringRequest(
				"URL (wszelkie zdjęcia o treści erotycznej spowodują zbanowanie)",
				"Wprowadź adres URL obrazu, który chcesz wyświetlić.",
		...
		self.sideArticleCaption = self:CreateTextEntry(right, nil, SScaleMin(28 / 3), TOP, 0, 0, 0, 0, nil, "ArialBold", 35, false, true, "", nil, string.upper("MAŁY CZŁOWIEK STOI PRZED SUMMIT 24"))
		self.sideArticleCaption:SetZPos(10)
		self.sideArticleCaption.bAlignLeft = true

		self.articleTitle2 = self:CreateTextEntry(right, nil, SScaleMin(58 / 3), TOP, 0, 0, 0, 0, nil, "AdobeCaslonProBoldSmaller", 26, false, true, "", nil, "Spotkanie szczytu")
		...
		self.article2Text = self:CreateTextEntry(right, 0, nil, FILL, 0, 0, 0, 0, nil, "AbrilTitlingRegular", 1700, true, true, "", nil, "Tekst artykułu 2 powinien być tutaj")
		...
		self.adImageButton = self:CreateSelectorButton(self.adPanel, SScaleMin(105 / 3), SScaleMin(105 / 3), LEFT, SScaleMin(47 / 3), SScaleMin(24 / 3), SScaleMin(54 / 3), SScaleMin(21 / 3), nil, "WYBIERZ OBRAZ", function()
			-- if self.cracked then LocalPlayer():Notify("You cannot add images to cracked newspapers!") return end
		...
		self.adTitle = self:CreateTextEntry(adTitleDescPanel, nil, SScaleMin(40 / 3), TOP, 0, 0, 0, 0, nil, "MinionProBoldAd", 11, false, true, "", nil, "TRIUMF")
		self.adTitle:SetZPos(17)

		self.adSubtitle = self:CreateTextEntry(adTitleDescPanel, nil, SScaleMin(50 / 3), TOP, 0, 0, 0, 0, nil, "MinionProItalic", 15, false, true, "", nil, "Kasyno i Salon")
		self.adSubtitle:SetZPos(18)

		self.adDesc = self:CreateTextEntry(self.adPanel, SScaleMin(125 / 3), nil, RIGHT, 0, SScaleMin(42 / 3), SScaleMin(42 / 3), SScaleMin(32 / 3), nil, "MinionProRegular", 80, true, true, "", nil, "Opis reklamy")
		...
		self:CreateOption(optionBot, LEFT, "PODGLĄD", function(button)
		...
		self:CreateOption(optionBot, RIGHT, "WYDRUKUJ", function(button)
	...
	local GenevanTribute = function()
		...
		self.headlineText = self:CreateTextEntry(self.mainPanel, SScaleMin(164 / 3), SScaleMin(64 / 3), TOP, 0, 0, 0, 0, nil, "UnifrakturTitle", 20, true, true, "", nil, "Trybuna Genewska")
		self.headlineText:SetZPos(-1)
		self.headlineText:SetContentAlignment(4)
		self.headlineText:DockMargin(self.surrounder:GetWide() * 0.21, SScaleMin(48 / 3), self.surrounder:GetWide() * 0.20, 0)
		...
		self.bigHeadline = self:CreateTextEntry(self.mainPanel, nil, SScaleMin(117 / 3), TOP, SScaleMin(33 / 3), SScaleMin(9 / 3), SScaleMin(33 / 3), 0, nil, "FjallaOneRegularLarge", 20, false, true, "", nil, string.upper("TERROR NA ULICACH"))
		self.bigHeadline:SetZPos(1)

		self.subHeadline = self:CreateTextEntry(self.mainPanel, nil, SScaleMin(60 / 3), TOP, SScaleMin(55 / 3), 0, SScaleMin(33 / 3), 0, nil, "FjallaOneRegularSmall", 52, false, true, "", nil, string.upper("MIESIĘCZNA BLOKADA OGŁOSZONA PO WYBUCHU ZAMIESZEK"))
		self.subHeadline:SetZPos(2)
		...
		self.imageButton = self:CreateSelectorButton(right, SScaleMin(650 / 3), SScaleMin(555 / 3), TOP, 0, 0, 0, 0, nil, "WYBIERZ OBRAZ", function()
			-- if self.cracked then LocalPlayer():Notify("You cannot add images to cracked newspapers!") return end
		...
		self.articleTitle = self:CreateTextEntry(left, nil, SScaleMin(60 / 3), TOP, 0, 0, 0, 0, nil, "AdobeCaslonProBold", 30, false, true, "", nil, "NAGŁÓWEK")
		self.articleTitle:SetZPos(4)
		self.articleTitle.bAlignLeft = true
		...
		self.articleText1 = self:CreateTextEntry(articleContentPanel, (SScaleMin(534 / 3) / 2) + SScaleMin(64 / 3), SScaleMin(60 / 3), LEFT, 0, 0, 0, 0, nil, "LusitanaSmall", 156, true, true, "", nil, "Tekst artykułu powinien być tutaj")
		...
		self.articleText2 = self:CreateTextEntry(articleContentPanel2, (SScaleMin(534 / 3) / 2) + SScaleMin(128 / 3), nil, LEFT, 0, 0, 0, 0, nil, "AbrilTitlingRegular", 1453, true, true, "", nil, "Tekst artykułu powinien być tutaj")
		...
		self.articleText3 = self:CreateTextEntry(articleContentPanel3, (SScaleMin(534 / 3) / 2) + SScaleMin(128 / 3), nil, LEFT, 0, 0, 0, 0, nil, "AbrilTitlingRegular", 190, true, true, "", nil, "Tekst artykułu powinien być tutaj")
		...
		self.advert = self:CreateTextEntry(adContents, nil, SScaleMin(60 / 3), TOP, 0, 0, 0, 0, nil, "LusitanaTitle", 30, false, true, "", nil, "REKLAMA")
		self.advert:SetZPos(12)

		self.advertName = self:CreateTextEntry(adContents, nil, SScaleMin(30 / 3), TOP, 0, 0, 0, 0, nil, "LusitanaItalic", 30, false, true, "", nil, "NAZWA REKLAMY!")
		...
		self.advertArticle = self:CreateTextEntry(articleContentPanel4, (SScaleMin(534 / 3) / 2) - SScaleMin(24 / 3), nil, LEFT, SScaleMin(9 / 3), 0, SScaleMin(3 / 3), 0, nil, "AbrilTitlingRegular", 279, true, true, "", nil, "Tekst artykułu powinien być tutaj")
		self.advertArticle.bAlignTopLeft = true

		self.rightHeadline = self:CreateTextEntry(rightContents,  SScaleMin(96 / 3), SScaleMin(48 / 3), TOP, SScaleMin(48 / 3), 0, 0, 0, nil, "LusitanaTitle", 30, false, true, "", nil, "NAGŁÓWEK")
		...
		self.articleText4 = self:CreateTextEntry(articleContentPanel5, (SScaleMin(534 / 3) / 2) + SScaleMin(2 / 3), nil, LEFT, 0, 0, 0, 0, nil, "AbrilTitlingRegular", 791, true, true, "", nil, "Tekst artykułu powinien być tutaj")
		...
		self.articleText5 = self:CreateTextEntry(articleContentPanel6, (SScaleMin(534 / 3) / 2) + SScaleMin(2 / 3), nil, LEFT, 0, 0, 0, 0, nil, "AbrilTitlingRegular", 160, true, true, "", nil, "Tekst artykułu powinien być tutaj")
		...
		self.imageButton2 = self:CreateSelectorButton(rightContents, SScaleMin(80 / 3), SScaleMin(190 / 3), TOP, SScaleMin(4 / 3), SScaleMin(8 / 3), SScaleMin(14 / 3), 0, nil, "WYBIERZ OBRAZ", function()
			-- if self.cracked then LocalPlayer():Notify("You cannot add images to cracked newspapers!") return end
		...
		self.sloganL = self:CreateTextEntry(sloganLR, SScaleMin(176 / 3), SScaleMin(16 / 3), LEFT, SScaleMin(44 / 3), 0, 0, 0, nil, "MinionProBold", 45, false, true, "", nil, string.upper("Trybuna Genewska"))
		self.sloganL:SetZPos(20)
		self.sloganL.bAlignRight = true
		self.sloganL.fixedHeight = false

		self.sloganR = self:CreateTextEntry(sloganLR, SScaleMin(204 / 3), SScaleMin(16 / 3), RIGHT, 0, 0, 0, 0, nil, "MinionProBold", 45, false, true, "", nil, string.upper("NAPRZÓD Z GENEWY"))
...
function PANEL:Populate(itemID, identifier, data, tNewspaper, dropdownPopulate)
	...
	if !self.cracked and tNewspaper.builder then
		...
		unionDatabaseLabel:SetText("Przesyłanie do bazy danych Unii")
...
function PANEL:CreateAdSelector(adPanel)
	...
	selector:Center()
	selector:SetTitle("Selektor obrazu reklamy")
...
function PANEL:TogglePreview(button)
	if self.previewEnabled then
		...
		for _, option in pairs(self.options) do
			...
			if option:GetText() == "WYJDŹ" then continue end
