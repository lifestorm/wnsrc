...
function PANEL:Init()
    ...
    self:CreateOption(optionBot, LEFT, "KOLOR KSIĄŻKI: ", nil, 0)
	...
	for colorName, bg in SortedPairs(colors) do
        self:CreateOption(optionBot, LEFT, colorName, function(button)
            if canClick then
                ...
                LocalPlayer():NotifyLocalized("Ustawiłeś kolor książki na "..string.utf8lower(colorName))

                timer.Simple(1, function()
                    canClick = true
                end)
            else
                LocalPlayer():NotifyLocalized("Nie możesz jeszcze tego kliknąć!")
            end
        end, 0)
    ...
    self:CreateOption(optionBot, RIGHT, "ZAPISZ", function(button)
...
function PANEL:Populate(itemID, identifier, data)
    ...
    self:CreateOption(self.optionPanels[2], RIGHT, "POZOSTAŁO EDYCJI: "..ix.config.Get("maxEditTimes"..Schema:FirstToUpper(identifier), 0) - data.editedTimes, nil)
end
