...
ITEM.name = "Pinezka do papieru"
ITEM.uniqueID = "pin"
ITEM.model = "models/items/crossbowrounds.mdl"
ITEM.width = 1
ITEM.height = 1
ITEM.description = "Pinezka. Służy do zawieszania papieru na ścianie czy tablicy."
	...
	OnRun = function(item)
		...
		if ( trTwo.Hit and !trTwo.Entity:IsPlayer() ) then
			if trace.Entity:GetClass() != "ix_item" then
				client:NotifyLocalized("Przypięty może zostać tylko przedmiot!")
				return false
			end
			...
			client:NotifyLocalized("Nie znaleziono rzeczy do przypięcia!")
			return false
		end
	end
}
