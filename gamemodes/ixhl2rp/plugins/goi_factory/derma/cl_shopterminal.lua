...
function PANEL:CreateNextPrev(parent, buttonFont, buttonAlign, prevFunc, nextFunc, bHorizontal, manualWidth)
	...
	CreateButton(nextButton, "następne", nil, buttonFont, buttonAlign)
	nextButton.DoClick = nextFunc

	local prevButton = parent:Add("DButton")
	CreateButton(prevButton, "poprzednie", nil, buttonFont, buttonAlign)
...
function PANEL:CreatePermitSelector(parent)
	...
	self.curPermit.Update = function(s)
		...
		if self.sPermits then
			self.sPermits:SetText(string.utf8upper("wybrane zezwolenia: ").."["..self.selectedPermits.."/3]")
		end
	end
end
...
function PANEL:BuildShopPanel(shop)
	...
	function()
		...
		upperLabel:SetText(string.utf8upper("terminal sklepu"))
		...
		if shop.shopName and shop.cost and shop.scReq and shop.rent then
			...
			self.shopInfo:SetText(string.utf8upper("")..shop.scReq.." SK | "..shop.cost..string.utf8upper(" kredytów").." | " .. shop.rent .. string.utf8upper(" czynszu"))
			...
			self.isPurchasable:SetText(isPurchasable and string.utf8upper("ten sklep można kupić") or string.utf8upper("tego sklepu nie można kupić"))
			...
			if isPurchasable then
				...
				CreateButton(self.selectPermit, "przełącz zezwolenie", nil, "WNTerminalMediumText", 5)
				...
				self.sPermits:SetText(string.utf8upper("wybranie zezwolenia: ").."["..self.selectedPermits.."/3]")
				...
				CreateButton(self.purchaseButton, "kup sklep", nil, "WNTerminalLargeText", 5)
