...
function PANEL:CreateLock()
	...
	function()
		...
		self.bottomWarning:SetText(string.utf8upper("ten terminal jest aktualnie używany"))
		...
		self.upperWarning:SetText(string.utf8upper("[PROSIMY O CIERPLIWOŚĆ // CZEKAJ NA SWOJĄ KOLEJ]"))
...
function PANEL:RequestCID()
	...
	function()
		...
		self.bottomLabel:SetText(string.utf8upper("włóż swój cid lub kartę cwu"))
		...
		self.upperLabel:SetText(string.utf8upper("[WYMAGANY CID, ABY KONTYNUOWAĆ // NALEŻY WŁOŻYĆ SWOJĄ KARTĘ]"))
...
function PANEL:CreateNextPrev(parent, buttonMat, buttonFont, buttonAlign, prevFunc, nextFunc, bHorizontal, manualWidth)
	...
	CreateButton(nextButton, "następne", buttonMat, buttonFont, buttonAlign)
	nextButton.DoClick = nextFunc

	local prevButton = parent:Add("DButton")
	CreateButton(prevButton, "poprzednie", buttonMat, buttonFont, buttonAlign)
...
function PANEL:CreateItemInteraction(pnl)
	...
	CreateButton(take, "weź z majątku", "buttonnoarrow.png")
	take:Dock(TOP)
	...
	CreateButton(sell, "sprzedaj", "buttonnoarrow.png")
	sell:Dock(FILL)
...
function PANEL:CreateItemPanel(parent, d1, d2, d3, d4)
	...
	upperLabel:SetText("OBECNY PRZEDMIOT:")
	upperLabel:SetTextColor(defClr)
	...
	pnl.OnItemChanged = function(s)
		...
		itemAmount:SetText(string.utf8upper("ilość: " .. pnl.itemData.amount))
	...
	if table.IsEmpty(self.items) then
		itemName:SetText(string.utf8upper("brak przedmiotów na stanie"))
		itemName:SizeToContents()
		itemAmount:SetText(string.utf8upper("ilość: brak"))
		itemAmount:SizeToContents()
	else
...
function PANEL:ProceedCreditOperation(operation, parent)
	...
	if operation == "payLoan" then
		labelText = string.utf8upper("spłać pożyczkę")
	elseif operation == "takeLoan" then
		labelText = string.utf8upper("weź pożyczkę")
	elseif operation == "transactionWithdraw" then
		labelText = string.utf8upper("wypłać kredyty")
	elseif operation == "transactionDeposit" then
		labelText = string.utf8upper("wpłać kredyty")
	end
	...
	credAmountLabel:SetText("ILOŚĆ KREDYTÓW")
	...
	confirmLabel:SetText("POTWIERDZENIEN")
	confirmLabel:SetTextColor(defClr)

	local confirmButton = confirm:Add("DButton")
	CreateButton(confirmButton, "POTWIERDŹ", "buttonnoarrow.png", nil, 5)
...
function PANEL:FillCreditInteraction(parent, proceedWith)
	...
	upperLabel:SetContentAlignment(5)
	upperLabel:SetFont("WNTerminalMediumText")
	upperLabel:SetText("INTERAKCJE KREDYTÓW")
	upperLabel:SetTextColor(defClr)
	...
	CreateButton(transactionWithdraw, "wypłać kredyty", "buttonnoarrow.png")
	...
	CreateButton(transactionDeposit, "wpłać kredyty", "smallerbuttonarrow.png")
	...
	CreateButton(loanPay, "spłać pożyczkę", "smallerbuttonarrow.png")
	loanPay:Dock(TOP)
	loanPay:SetTall(cInteraction:GetTall() / 5)
	loanPay:DockMargin(8, 5, 8, 5)

	local loanTake = cInteraction:Add("DButton")
	CreateButton(loanTake, "weź pożyczkę", "smallerbuttonarrow.png")
	loanTake:Dock(TOP)
...
function PANEL:ProceedBudgetOperation(parent)
	...
	upperLabel:SetContentAlignment(5)
	upperLabel:SetFont("WNTerminalMediumText")
	upperLabel:SetText("OBECNY BUDŻET FRAKCJI")
	upperLabel:SetTextColor(defClr)
	...
	confirmLabel:SetContentAlignment(5)
	confirmLabel:SetFont("WNTerminalSmallText")
	confirmLabel:SetText("POTWIERDZENIE")
	confirmLabel:SetTextColor(defClr)
...
function PANEL:FillBudgetInteraction(parent, bData)
	...
	if #self.factionList == 0 then
		...
		errorLabel:SetText("BRAK AKTYWNYCH FRAKCJI")
	...
	upperLabel:SetContentAlignment(5)
	upperLabel:SetFont("WNTerminalMediumText")
	upperLabel:SetText("BUDŻETY FRAKCJI")
	upperLabel:SetTextColor(defClr)
	...
	curFaction:SetText("[OBECNA FRAKCJA]")
	...
	if !self:IsCombine() then
		...
		self.pickedFactionBudget:SetText("OBECNY BUDŻET: " .. self.currentFaction.credits .. " KREDYTÓW")
...
function PANEL:BuildCreditInfo(parent)
	...
	bottomLabel:SetText("DOSTĘPNE KREDYTY")
...
function PANEL:BuildStockInfo(parent)
	...
	bottomLabel:SetContentAlignment(5)
	bottomLabel:SetFont("WNTerminalMediumText")
	bottomLabel:SetText("PRZEDMIOTY W MAJĄTKU MIASTA")
	bottomLabel:SetTextColor(defClr)
...
function PANEL:BuildLoanInfo(parent)
	...
	bottomLabel:SetContentAlignment(5)
	bottomLabel:SetFont("WNTerminalMediumText")
	bottomLabel:SetText("OBECNE ZADŁUŻENIE MIASTA")
	bottomLabel:SetTextColor(defClr)
...
function PANEL:FillCityInfo(parent)
	...
	upperLabel:SetContentAlignment(5)
	upperLabel:SetFont("WNTerminalMediumText")
	upperLabel:SetText("INFORMACJE O MIEŚCIE")
	upperLabel:SetTextColor(defClr)
...
function PANEL:FillBudgetTypes(parent)
	...
	upperLabel:SetContentAlignment(5)
	upperLabel:SetFont("WNTerminalMediumText")
	upperLabel:SetText("INTERAKCJE BUDŻETU")
	upperLabel:SetTextColor(defClr)
	...
	curInteract:SetText("WYBRANA INTERAKCJA")
...
function PANEL:BuildCitySelector(parent)
	...
	self.sCityLabel:SetText("MIASTO-" .. self.selectedCity.id)
	...
	self.sCityLabel.Update = function(s)
		s:SetText("MIASTO-" .. self.selectedCity.id)
...
function PANEL:CreateMarketItemViewer(parent, d1, d2, d3, d4)
	...
	self.marketViewer.OnItemChanged = function(s)
		...
		itemName:SizeToContents()
		itemAmount:SetText(string.utf8upper("ilość: " .. self.marketViewer.itemData.amount))
		itemAmount:SizeToContents()
...
local function GetCurrentState(itemData)
	if itemData.priceMulptiplicationTD and itemData.amount <= itemData.priceMulptiplicationTD then
		return {
			text = string.utf8upper("niedobór"),
			color = redClr
		}
	elseif itemData.priceReductionTD and itemData.amount >= itemData.priceReductionTD then
		return {
			text = string.utf8upper("dostatek"),
			color = greenClr
		}
	else
		return {
			text = string.utf8upper("normalny"),
			color = defClr
		}
	end
end
...
local function CanAfford(itemPrice, mainCity)
	if itemPrice > mainCity.credits then
		return string.utf8upper("nie stać cię\n na ten przedmiot")
	else
		return string.utf8upper("stać cię\n na ten przedmiot")
	end
end
...
function PANEL:FillItemInformation(parent, itemData)
	...
	if (!itemData) then
		...
		errorLabel:SetContentAlignment(5)
		errorLabel:SetFont("WNTerminalMediumText")
		errorLabel:SetText("BRAK INFORMACJI O PRZEDMIOCIE")
		errorLabel:SetTextColor(defClr)
	...
	selectedCityLabel:SetText("MIASTO-" .. self.selectedCity.id)
	...
	self.currentPrice:SetText(string.utf8upper("obecna cena: ") .. GetCurrentPrice(itemData))
	...
	self.defaultPrice:SetText(string.utf8upper("domyślna cena: ") .. itemData.price)
	...
	self.cityCredits:SetText(string.utf8upper("kredyty miasta: ") .. self.selectedCity.credits)
	...
	self.currentState:SetText(string.utf8upper("obecny stan: \n") .. curState.text)
	...
	yourCityLabel:SetText(string.utf8upper("twoje miasto"))
	...
	self.mCredits:SetText(string.utf8upper("kredyty miasta: ") .. self.mainCity.credits)
	...
	self.mItemAmount:SetText(self.mainCity.items[itemData.id] and string.utf8upper("ilość: ") .. self.mainCity.items[itemData.id].amount or string.utf8upper("brak tego typu przedmiotów w majątku"))
...
function PANEL:ProceedItemOperation(operation, parent, pWide, itemData)
	...
	if (!itemData) then
		...
		errorLabel:SetContentAlignment(5)
		errorLabel:SetFont("WNTerminalMediumText")
		errorLabel:SetText("BRAK DANYCH O PRZEDMIOCIE")
		errorLabel:SetTextColor(defClr)
		errorLabel:SizeToContents()
		return
	end
	...
	if operation == "addToCart" then
		labelText = string.utf8upper("dodaj do koszyka")
	elseif operation == "sellItem" then
		labelText = string.utf8upper("sprzedaj przedmiot")
	end
	...
	itemAmountLabel:SetText("ILOŚĆ PRZEDMIOTÓW")
...
function PANEL:FillItemInteraction(parent, itemData)
	...
	if !self.marketViewer:GetItem() then
		...
		CreateButton(refreshButton, "ODŚWIEŻ MIASTA", "buttonnoarrow.png", nil, 5)
	...
	CreateButton(cartButton, "DODAJ DO KOSZYKA", "buttonnoarrow.png", nil, 5)
	cartButton:Dock(TOP)
	...
	CreateButton(sellButton, "SPRZEDAJ PRZEDMIOT", "buttonnoarrow.png", nil, 5)
	...
	sellButton.DoClick = function(s)
		...
		if self.mainCity.items[self.marketViewer:GetItem().id] then
			...
			LocalPlayer():NotifyLocalized("Twoje miasto nie ma tego przedmiotu.")
		end
	end
end
...
function PANEL:CreateMarketPanel(cData)
	...
	if #self.selectorCities == 0 then
		...
		errorLabel:SetText("BRAK DOSTĘPNYCH MIAST W TYM MOMENCIE")
		errorLabel:SetTextColor(defClr)
		return
	end
	...
	upperLabel:SetContentAlignment(5)
	upperLabel:SetFont("WNTerminalLargeText")
	upperLabel:SetText("[EKSPORTUJ // IMPORTUJ]")
	upperLabel:SetTextColor(defClr)
	...
	itemLabel:SetText("OBECNY PRZEDMIOT")
	...
	cityLabel:SetText("OBECNE MIASTO")
	...
	interactionLabel:SetText("INTERAKCJE")
...
function PANEL:CreateCartItemViewer(parent, d1, d2, d3, d4)
	...
	self.cartViewer.OnItemChanged = function(s)
		...
		itemName:SizeToContents()
		itemAmount:SetText(string.utf8upper("ilość w koszyku: " .. s.itemAmount))
		itemAmount:SizeToContents()
		itemCity:SetText(string.utf8upper("miasto: " .. s.itemCity))
...
function PANEL:CreateCheckout(parent, cData)
	...
	if #self.cart == 0 then
		...
		emptyCart:SetText(string.utf8upper("twój koszyk jest pusty"))
	...
	intLabel:SetText(string.utf8upper("interakcje koszyka"))
	...
	CreateButton(clearCart, "WYCZYŚĆ KOSZYK", "buttonnoarrow.png", nil, 5)
	clearCart:Dock(TOP)
	...
	CreateButton(removeItem, "USUŃ WYBRANY PRZEDMIOT", "buttonnoarrow.png", nil, 5)
	...
	checkoutLabel:SetText(string.utf8upper("zamów"))
	...
	itemsInCart:SetText(string.utf8upper("przedmioty w koszyku: " .. self:GetItemCount()))
	...
	citiesCount:SetText(self:GetCityCount() == 1 and string.utf8upper("importowanie z: " .. self:GetCityCount() .. " miasta") or string.utf8upper("importowanie z: " .. self:GetCityCount() .. " miasta"))
	...
	totalPrice:SetText(string.utf8upper("łączna cena: " .. tPrice .. " kredytów"))
	...
	cityCredits:SetText(string.utf8upper("obecny fundusz miasta: " .. cData.credits .. " kredytów"))
	...
	if CanCityAfford(cData.credits, tPrice) then
		...
		confirmationLabel:SetText(string.utf8upper("potwierdzić działanie?"))
		...
		CreateButton(confirmButton, "POTWIERDŹ", "buttonnoarrow.png", "WNTerminalMediumText", 5)
		confirmButton:Dock(FILL)
		...
		noCreditsLabel:SetText(string.utf8upper("nie stać cię na to"))
...
function PANEL:PopulateWorkshift(data)
	...
	if IsValid(self.workshiftLabel) then
		self.workshiftLabel:SetText(GetNetVar("WorkshiftStarted", false) and string.utf8upper("cykl pracy jest online") or string.utf8upper("cykl pracy jest offline"))
	...
	if IsValid(self.participantLabel) then
		if #self.workshiftData.participants > 0 then
			self.participantLabel:SetText(string.utf8upper("wybrany uczestnik [".. self.workshiftNextPrev.pos .."/" .. #self.workshiftData.participants .. "]:"))
		else
			self.participantLabel:SetText(string.utf8upper("wybrany uczestnik [0/0]:"))
		end
	end

	if IsValid(self.participantName) then
		self.participantName:SetText(self.workshiftData.participants[self.workshiftNextPrev.pos] and self.workshiftData.participants[self.workshiftNextPrev.pos][2] or "/BRAK DANYCH/")
	end
...
function PANEL:CreateDataSelector(parent, name, isInt, data, confirmFunction)
	...
	CreateButton(confirmButton, "POTWIERDŹ", "buttonnoarrow.png", "WNTerminalMediumSmallerText", 5)
	confirmButton:Dock(FILL)
...
function PANEL:BuildRewards(parent)
	...
	self.couponSelector = self:CreateDataSelector(self.coupons, "Kupon", false, self.couponList, function()
		self:UpdateWorkshiftRewards("coupon", self.couponSelector.data == "BRAK" and false or self.couponSelector.data)
	end)
	...
	self.scSelector = self:CreateDataSelector(self.socialCredits, "Kredyty Społeczne", true, nil, function()
	...
	if participant then
		...
		self.couponSelector:SetValue(self.workshiftData.rewards[participant] and self.workshiftData.rewards[participant]["coupon"] and self.workshiftData.rewards[participant]["coupon"] or "BRAK")
	end
end
...
function PANEL:BuildShift(parent)
	...
	CreateButton(beginShift, "ROZPOCZNIJ CYKL", "buttonnoarrow.png", "WNTerminalMediumText", 5)
	...
	CreateButton(endShift, "ZAKOŃCZ CYKL", "buttonnoarrow.png", "WNTerminalMediumText", 5)
	endShift:Dock(LEFT)
	...
	CreateButton(pauseReg, "WSTRZYMAJ REJ.", "buttonnoarrow.png", "WNTerminalMediumText", 5)
	pauseReg:Dock(LEFT)
	...
	CreateButton(toggleBroadcast, "TRANSMISJA", "buttonnoarrow.png", "WNTerminalMediumText", 5)
	...
	participantsLabel:SetText(string.utf8upper("uczestnicy cyklu pracy"))
	...
	CreateButton(refreshButton, "ODŚWIEŻ INFORMACJE", "buttonnoarrow.png", "WNTerminalMediumText", 5)
	refreshButton:Dock(TOP)
	...
	self.participantName:SetText("/BRAK DANYCH/")
	...
	local incrementFunc = function(s)
		...
		if self.workshiftData.participants[self.workshiftNextPrev.pos + 1] then
			...
			self.participantLabel:SetText(string.utf8upper("wybrany uczestnik [".. self.workshiftNextPrev.pos .."/" .. #self.workshiftData.participants .. "]:"))
			self.participantName:SetText(self.workshiftData.participants[self.workshiftNextPrev.pos][2] or "/BRAK DANYCH/")
	...
	self.options = {
		["CITY STOCK"] = function()
			...
			function()
				...
				self.stockLabel:SetText(string.utf8upper("[majątek miasta]"))
...
function PANEL:AuthError()
	...
	function()
		...
		self.bottomLabel:SetFont("WNTerminalLargeText")
		self.bottomLabel:SetText(string.utf8upper("błąd autoryzacji!"))
		...
		self.upperLabel:SetContentAlignment(5)
		self.upperLabel:SetFont("WNTerminalMediumText")
		self.upperLabel:SetText(string.utf8upper("[PROSZĘ CZEKAĆ NA PRZYBYCIE CIVIL PROTECTION // NIE ODDALAJ SIĘ]"))
