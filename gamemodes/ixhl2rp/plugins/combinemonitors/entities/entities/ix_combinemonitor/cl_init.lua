...
function ENT:Draw()
    ...
    local visorText =  !isstring(GetNetVar("visorText")) and "SOCIO-STABILNOŚĆ NIENARUSZONA" or GetNetVar("visorText")
    local visorTextNoSocio = string.Replace(visorText, "SOCIO-STABILNOŚĆ", "")
    ...
    cam.Start3D2D(position + forward + trueR + thisU, angles, 0.25 * (self.small and smallScale or self.tv and tvScale or 1))
        ...
        draw.DrawText( "SOCIO-STABILNOŚĆ", "CombineMonitorLarge", w / 2 - (self.tv and 50 or 25), -30, Color( 255, 255, 255, alpha ), TEXT_ALIGN_LEFT )
        ...
        draw.DrawText( "STATUS CYKLI", "CombineMonitorLarge", w / 2 - (self.tv and 50 or 25), 80, Color( 255, 255, 255, alpha ), TEXT_ALIGN_LEFT )
    ...
    if (halo.RenderedEntity() != self) then
        cam.Start3D2D(position + forward + thisR + thisU, angles, 0.25 * (self.small and smallScale or self.tv and tvScale or 1))
            PLUGIN.clip:Scissor2D(widthBox, h * 2)
                ...
                local text2 = (!workshiftStatus and "CYKLE NIEAKTYWNE" or "CYKLE AKTYWNE").."     ///     "
        ...
        cam.Start3D2D(position + forward + thisR2 + thisU2, angles, 0.25 * (self.small and smallScale or self.tv and tvScale or 1))
            PLUGIN.clip:Scissor2D(w * 4.3, h * 2)
                draw.DrawText( "TRANSMISJE", "CombineMonitorLarge", w / 2, 5, Color( 255, 255, 255, alpha ), TEXT_ALIGN_LEFT )
