...
function Schema:PlayerLoadedCharacter(client, character, oldCharacter)
	if (client:IsDispatch()) then
		ix.combineNotify:AddNotification("LOG:// Odszyfrowywanie przychodzącego węzła połączenia z adresu 144.76.222.66")

		timer.Simple(1, function()
			ix.combineNotify:AddNotification("NTC:// Węzeł Sztucznej Inteligencji Dispatch online", Color(255, 255, 0, 255))
		end)
	elseif (oldCharacter and oldCharacter:GetFaction() == FACTION_OVERWATCH) then
		ix.combineNotify:AddNotification("NTC:// Węzeł sztucznej inteligencji Dispatch offline", Color(255, 255, 0, 255))
	elseif (client:IsCombine() or (oldCharacter and oldCharacter:IsCombine())) then
		ix.combineNotify:AddNotification("LOG:// Rekonstrukcja manifestu Overwatch", Color(50, 100, 175))
	end
end
...
function Schema:PlayerDeath(client, inflicter, attacker)
	if (client:HasActiveTracker() or client:IsCombineScanner()) then
		...
		ix.combineNotify:AddNotification("NTC:// Pobieranie utraconego biosygnału")
		ix.combineNotify:AddImportantNotification("WRN:// Utracono biosygnał dla jednostki " .. client:GetCombineTag(), nil, client, client:GetPos())
...
function Schema:PlayerHurt(client, attacker, health, damage)
	...
	if (client:HasActiveTracker() and (client.ixTraumaCooldown or 0) < CurTime()) then
		if (health < 25) then
			ix.combineNotify:AddNotification("WRN:// Spadek parametrów życiowych - zwróć się o pomoc medyczną", Color(255, 0, 0, 255), nil, nil, client)
		else
			local text = damage > 50 and "Poważny" or "Zewnętrzny"
			ix.combineNotify:AddNotification("WRN:// Wykryto " .. text .. " uraz", Color(255, 0, 0, 255), nil, nil, client)
		end
...
function Schema:CanPlayerJoinClass(client, class, info)
	if (client:IsRestricted()) then
		client:Notify("Nie możesz zmienić klasy, gdy jesteś związany!")
...
netstream.Hook("ViewObjectivesUpdate", function(client, text)
	if (client:GetCharacter() and hook.Run("CanPlayerEditObjectives", client)) then
		...
		ix.combineNotify:AddNotification("LOG:// Pobieranie zaktualizowanego manifestu celów Overwatch (REWIZJA  " .. date:spanseconds() .. ")")
...
netstream.Hook("SearchDecline", function(client)
	...
	client.ixSearchRequest:NotifyLocalized(hook.Run("GetCharacterName", client, "ic") or client:GetName().." nie pozwala być przeszukanym.")
...
netstream.Hook("SearchTimeout", function(client)
	...
	client.ixSearchRequest:NotifyLocalized(hook.Run("GetCharacterName", client, "ic") or client:GetName().." nie odpowiedział na zażądanie.")
	client.ixSearchRequest = nil
end)
