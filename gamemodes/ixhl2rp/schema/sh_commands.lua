...
do
	ix.command.Add("CharSearch", {
		description = "Przeszukaj gracza, na którego się patrzysz. Wyślesz zażądanie pozwolenia, jeśli nie jest związany.",
			...
			if (IsValid(target) and target:IsPlayer()) then
				...
				elseif (!IsValid(target.ixSearchRequest)) then
					...
					elseif (count == 2) then
						...
						if (os.time() < client.ixLastSearchRequest.time) then
							client:NotifyLocalized("Odczekaj 15 sekund zanim wyślesz trzecie żądanie!")
							return
						end
					end
					client.ixLastSearchRequest.time = os.time() + 15

					client:NotifyLocalized("Żądanie przeszukania zostało wysłane.")
...
ix.command.Add("UnStuck", {
	...
	OnRun = function(self, client)
		...
		client:RequestString("Użyj, jeśli utkniesz lub zostaniesz zabity przez prop. Przenosi z powrotem do miejsca odrodzenia i resetuje HP. Nie nadużywać.", "     Aby kontynuować, rozwiąż następujące zadanie matematyczne: " .. unstuckNumberOne .. " " .. unstuckOperator .. " " .. unstuckNumberTwo .. " = ?     ", function(text)
			...
			if (text and isnumber(text)) then
				...
				if (text == answer) then
					client:RequestString("Użyj, jeśli utkniesz lub zostaniesz zabity przez prop. Przenosi z powrotem do miejsca odrodzenia i resetuje HP. Nie nadużywać.", "Wpisz poniżej powód, dla którego chcesz użyć komendy", function(reasontext)
						for _, player in ipairs(player.GetAll()) do
							if (player:IsAdmin()) then
								player:ChatNotifyLocalized("unstuckAdmins", client:GetName(), client:SteamName(), client:GetArea(), reasontext)
								ix.log.Add(client, "command"," /UnStuck powodem do korzystania z niego było: "..reasontext)
							end
						end
...
ix.command.Add("SetOwnName", {
	description = "Ustaw imię własnej postaci, jeśli jest to dozwolone.",
	arguments = ix.type.text,
	OnRun = function(self, client, name)
		if (client:Team() != FACTION_OTA) then
			client:Notify("Nie możesz ustawić własnego imienia!")

			return
		end

		client:GetCharacter():SetName(name)
		client:Notify("Ustawiłeś swoje imię na " .. name .. ".")
	end
})
