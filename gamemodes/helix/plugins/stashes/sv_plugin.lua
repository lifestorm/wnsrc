...
ix.log.AddType("stashMoneyTake", function(client, name, amount, total)
    return string.format("%s a pris %d %s de sa réserve '%s' (il reste %d %s).",
...
ix.log.AddType("stashMoneyGive", function(client, name, amount, total)
    return string.format("%s a donné %d %s à sa réserve '%s' (%d %s stocké maintenant).",
        client:GetName(), amount, ix.currency.plural, name, total, ix.currency.plural)
end)
...
ix.log.AddType("stashOpen", function(client, stashName, bIsAdmin)
    return string.format("%s %s la réserve '%s'.", client:Name(), bIsAdmin and "admin-viewed" or "opened", stashName)
end, FLAG_NORMAL)

ix.log.AddType("stashClose", function(client, stashName)
    return string.format("%s a fermé la réserve '%s'.", client:Name(), stashName)
end, FLAG_NORMAL)

ix.log.AddType("stashLocationReset", function(client, stashName)
    return string.format("%s leur réserve '%s' n'existe plus, emplacement réinitialisé.", client:Name(), stashName)
end, FLAG_NORMAL)

ix.log.AddType("stashName", function(client, name)
    return string.format("%s a défini un nom de réserve pour'%s'.", client:Name(), name)
end)

ix.log.AddType("stashCreate", function(client, stashModel)
    return string.format("%s a créé une réserve (%s).", client:Name(), stashModel)
end, FLAG_NORMAL)
