	...
	function COMMAND:OnRun(client, text)
		...
		for k, v in pairs(texts) do
			if (v.map == currentMap and v.pos:DistToSqr(pos) < 50 * 50) then
				client:Notify("Diğer yazıya çok yakın!")
		...
		if (!CAMI.PlayerHasAccess(client, "Helix - Basic Admin Commands")) then
			if (CAMI.PlayerHasAccess(client, "Helix - Increased Text Limit")) then
				if (count >= ix.config.Get("staticTextIncreasedLimit")) then
					client:Notify("Statik yazı limitine ulaştın, yenisini eklemeden önce en az birini silin.")

					return
				end
			else
				client:Notify("Halihazırda statik bir metniniz mevcut, yenisini eklemeden önce öncekini silin.")
		...
		client:Notify("Statik yazı eklendi.")
			...
			for k, v in pairs(texts) do
				if (v.pos:DistToSqr(pos) < 50 * 50) then
					if (v.steamid == client:SteamID() or CAMI.PlayerHasAccess(client, "Helix - Basic Admin Commands")) then
						...
						client:Notify("Statik yazı kaldırıldı.")

						ix.log.Add(client, "staticTextRemoved", v)
					else
						client:Notify("Bu metni kaldırmak için yeterli izniniz yok.")
					end
...
do
	...
	function COMMAND:OnRun(client)
		...
		if (textCount > 0) then
			...
			client:Notify(textCount .. " adet statik yazı sildiniz.")
		else
			client:Notify("Herhangi bir statik yazınız yok!")
		end
	end
