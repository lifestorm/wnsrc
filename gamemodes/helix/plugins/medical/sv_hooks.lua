	...
	if (IsValid(client) and client:IsPlayer()) then
		...
		if (character:GetBleedout() == -1) then
			...
			if (client:HasActiveTracker()) then
				ix.combineNotify:AddImportantNotification("WRN:// Jednostka " .. client:GetCombineTag() .. " osiągnęła krytyczne parametry życiowe", nil, client, client:GetPos(), nil, client:GetCombineTag())
			end
