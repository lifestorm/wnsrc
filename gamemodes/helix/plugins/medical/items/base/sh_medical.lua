...
function ITEM:GetBoostAppend()
	...
	if (ix.special and self.boosts) then
		boostAppend[#boostAppend + 1] = "KRÓTKOTERMINOWE WZMOCNIENIA:\n"

		if (self.boosts.strength) then
			boostAppend[#boostAppend + 1] = string.format("Siła: %d\n", self.boosts.strength)
		end
		if (self.boosts.agility) then
			boostAppend[#boostAppend + 1] = string.format("Zręczność: %d\n", self.boosts.agility)
		end
		if (self.boosts.intelligence) then
			boostAppend[#boostAppend + 1] = string.format("Inteligencja: %d\n", self.boosts.intelligence)
		end
		if (self.boosts.perception) then
			boostAppend[#boostAppend + 1] = string.format("Percepcja: %d", self.boosts.perception)
		end
	end
...
function ITEM:GetHealingAppend()
	...
	if (character and self.healing) then
		healingAppend[#healingAppend + 1] = "LECZENIE:\n"
		if (self.healing.bandage) then
			healingAppend[#healingAppend + 1] = string.format("Bandaż: %dHP\n", self.healing.bandage * (1 + character:GetSkillScale("bandage_skill")))
		end
		if (self.healing.disinfectant) then
			healingAppend[#healingAppend + 1] = string.format("Środek dezynfekujący: %dHP\n", self.healing.disinfectant * (1 + character:GetSkillScale("disinfectant_skill")))
		end
		if (self.healing.painkillers) then
			healingAppend[#healingAppend + 1] = string.format("Środki przeciwbólowe: %dHP", self.healing.painkillers)
		end
	end
...
ITEM.functions.use = {
	...
	OnCanRun = function(item)
		...
		if (timer.Exists("combattimer" .. item.player:SteamID64()) and !item.usableInCombat) then
			item.player:Notify("Nie możesz tego użyć podczas walki.")
			return false
		end
	...
	OnRun = function(item)
        ...
        item.player:SetAction("Aplikowanie medycyny...", 3, function()
