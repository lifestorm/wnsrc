...
function PANEL:Init()
	...
	textStandard(nlrTitle, Color(234, 236, 233, 255), "WNBleedingTitleNoClamp", 78 + padding, string.utf8upper("nie żyjesz")) -- 78 icon height

	local nlrText = textPanel:Add("DLabel")
	local nlrText2 = textPanel:Add("DLabel")
	local nlrText3 = textPanel:Add("DLabel")
	textStandard(nlrText, Color(200, 200, 200, 255), "WNBleedingTextNoClamp", -padding, "Obowiązuje zasada nowego życia, ekwipunek zostaje zresetowany")
	textStandard(nlrText2, Color(200, 200, 200, 255), "WNBleedingTextNoClamp", -padding, "a poziomy umiejętności zostają obniżone.")

	textStandard(nlrText3, Color(200, 200, 200, 255), "WNBleedingTextBold", -padding + 10, "Podczas trwania tego ekranu możesz wykonać ostatnią akcję /me.")

	local okayButton = textPanel:Add("DButton")
	okayButton:SetSize(self:GetWide(), SScaleMin(46 / 3))
	okayButton:Dock(TOP)
	okayButton:SetText("Okej")
