...
function PANEL:Init()
	...
	textPanel:SetSize(SScaleMin(545 / 3), SScaleMin(377 / 3))
	textPanel:Center()
	...
	textStandard(bleedingTitle, Color(234, 236, 233, 255), "WNBleedingTitleNoClamp", 61 + padding, string.utf8upper("wykrwawiasz się")) -- 61 icon height

	local bleedingText = textPanel:Add("DLabel")
	textStandard(bleedingText, Color(200, 200, 200, 255), "WNBleedingTextNoClamp", -padding, "Jesteś nieprzytomny i potrzebujesz bandaży, aby przeżyć.")
	...
	function self.timeLeftText:SetTime(time)
		self:SetText(string.utf8upper(time > 120 and math.Round(time / 60).." min" or math.Round(time, 1).." s"))
	end

	local controlText = textPanel:Add("DLabel")
	textStandard(controlText, Color(200, 200, 200, 255), "WNBleedingTextNoClamp", -padding, "Przytrzymaj E, aby zaakceptować swoją śmierć.")
	...
	textStandard(hpInfoText, Color(200, 200, 200, 255), "TitlesFontNoBoldNoClamp", -padding + 15, "Potrzebujesz "..requiredHealth.."HP, aby się podnieść")

	local currentHPText = textPanel:Add("DLabel")
	textStandard(currentHPText, Color(255, 78, 79, 255), "TitlesFontNoBoldNoClamp", -padding + 5, "Obecne HP: "..LocalPlayer():Health())
