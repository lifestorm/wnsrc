...
function PANEL:Init()
	...
	textStandard(bleedingTitle, Color(234, 236, 233, 255), "WNBleedingTitleNoClamp", 61 + padding, string.utf8upper("kanıyorsun")) -- 61 icon height

	local bleedingText = textPanel:Add("DLabel")
	textStandard(bleedingText, Color(200, 200, 200, 255), "WNBleedingTextNoClamp", -padding, "Bilincini kaybettin ve bandaja ihtiyacın var.")
	...
	function self.timeLeftText:SetTime(time)
		self:SetText(string.utf8upper(time > 120 and math.Round(time / 60).." dakika" or math.Round(time, 1).." saniye"))
	end

	local controlText = textPanel:Add("DLabel")
	textStandard(controlText, Color(200, 200, 200, 255), "WNBleedingTextNoClamp", -padding, "Karakterinizi öldürmek için E tuşuna basılı tutun.")
	...
	textStandard(hpInfoText, Color(200, 200, 200, 255), "TitlesFontNoBoldNoClamp", -padding + 15, "Ayağa kalkmak için "..requiredHealth.."HP'ye ihtiyacınız var.")

	local currentHPText = textPanel:Add("DLabel")
	textStandard(currentHPText, Color(255, 78, 79, 255), "TitlesFontNoBoldNoClamp", -padding + 5, "Mevcut HP: "..LocalPlayer():Health())
