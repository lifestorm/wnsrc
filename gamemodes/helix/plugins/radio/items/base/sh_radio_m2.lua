...
ITEM.description = "Duże, potężne radio o dalekim zasięgu. Pozwala na komunikacje na dużym dystansie."
...
if (CLIENT) then
	...
	function ITEM:PopulateTooltip(tooltip)
		...
		if (channels) then
			...
			chTip:SetText("Kanały: "..table.concat(channelNames, ", "))
    ...
    multiOptions = function(item, player)
		...
		for i = 1, item.maxFrequencies do
			options[i] = {name = "Freq "..i, OnClick = function(itemTbl)
                ...
                Derma_StringRequest("Ustaw częstotliwość", "Wprowadź częstotliwość jaką chcesz ustawić:", freqText, function(text)
                    ...
                    if (string.len(text) == 2 and newFreq and newFreq >= 10) then
                        ...
                        player:Notify("Wprowadź częstotliwość pomiędzy 100 a 999")
                    end
                end, nil, "Ustaw", "Anuluj")
                return false
            end}
		end
