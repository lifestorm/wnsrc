...
function PANEL:Init()
	...
	self:Center()
    self:SetTitle("Quiz - Aby stworzyć postać, musisz odpowiedzieć na kilka pytań.")
	...
	self.loadingLabel:SetText("Wczytywanie pytań quizu...")
...
function PANEL:CreateQuizContent()		
	...
	for k, table in pairs(PLUGIN.quizlist) do
		...
		answerPanel:SetValue("Wybierz odpowiedź")
	...
	panelText:SetText("Nie jesteś pewien odpowiedzi? Sprawdź nasze forum, aby uzyskać więcej informacji.        ")
...
function PANEL:GetAnswers()
	...
	for _, v in pairs(self.comboboxes) do
		if v:GetSelected() and v:GetSelected() != "" and v:GetSelected() != "Wybierz odpowiedź" then
