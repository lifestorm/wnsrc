...
function PANEL:Init()
	...
	titleLabel:SetText(string.utf8upper("Postacie"))
	...
	for i = 1, #ix.characters do
		...
		if (faction.name) == "Overwatch Transhuman Arm" then
			factionText:SetText("ŻOŁNIERZ OVERWATCH")
		else
	...
	if (ix.config.Get("CharCreationDisabled", false) and !CAMI.PlayerHasAccess(LocalPlayer(), "Helix - Character Creation Bypass") and #ix.characters == 0) then
        Derma_Query( "Czy chcesz połączyć się z serwerem miasta?", "Nie masz żadnych postaci w tej lokalizacji.", "TAK", function()
	...
	self.back:SetText(string.utf8upper("Wróć"))
...
-- character deletion panel
	...
	deleteTitle:SetText(string.utf8upper("czy na pewno?"))
	...
	deleteText:SetText("Ta postać zostanie nieodwracalnie usunięta!")
	...
	yes:SetText(string.utf8upper("tak"))
	...
	no:SetText(string.utf8upper("nie"))
