...
function PANEL:CreateFactionTitles()
	...
	self.titleLabel:SetText(string.utf8upper("Frakcje"))
...
function PANEL:CreateFactionBack()
	...
	self.factionBack:SetText(string.utf8upper("Powrót"))
...
function PANEL:CreateBackgroundButton()
	...
	self:CreateMainButton(self.backgroundButton, "tło postaci", "background")
	self.backgroundButton.DoClick = function()
		...
		self:CreateRightMenuTextPanel("wybór tła postaci", 60)
		...
		if self.faction:GetNoBackground(LocalPlayer()) != true then
			if self.faction.name == "Obywatel" then
				...
				self:CreateBackgroundSelectionPanels("willardnetworks/mainmenu/charcreation/local.png", 34, 61, "Lokalny Obywatel", "Mieszkasz tu od całkiem długiego czasu, może nawet od czasu przed okupacją. Jakimś cudem miałeś te szczęście i nigdy nie zostałeś relokowany. Znasz już to miasto praktycznie na wylot.", 0)
				...
				self:CreateBackgroundSelectionPanels("willardnetworks/mainmenu/charcreation/outcast.png", 55, 55, "Wyrzutek", "Zawsze w ruchu i zawsze w cieniu. Żyjesz na własną rękę w najbiedniejszych rejonach miasta, sprawnie unikając ciężkiej ręki Kombinatu.", 5)
			...
			noBackground:SetText("Wybór tła nie jest dostępny dla tej frakcji")
...
function PANEL:CreateAttribute(icon, wIcon, hIcon, title, desc, attribute, bSkill, minorBoost, majorBoost)
	...
	if (bSkill) then
		...
		boostedLevels:SetText("Poziomy: +"..attBoostSum)
...
function PANEL:CreateAttributesButton()
	...
	self:CreateMainButton(self.attributesButton, "atrybuty", "attributes")
	self.attributesButton.DoClick = function()
		...
		panelText:SetText(string.utf8upper("wybór atrybutów"))
		...
		if self.faction.noAttributes then
			...
			noAttributesAvailable:SetText("Wybór atrybutów nie jest dostępny dla tej frakcji")
			...
			self.attributesRemaining:SetText(self:GetPointsLeft()..string.utf8upper(" pozostałych punktów"))
			self.attributesRemaining:SizeToContents()

			self:CreateAttribute("willardnetworks/mainmenu/charcreation/strength.png", 45, 61, "siła", "Znaczne wzmocnienie broni \nNiewielkie wzmocnienie prędkości ruchu i wytwarzania", "strength")
			self:CreateAttribute("willardnetworks/mainmenu/charcreation/perception.png", 45, 30, "percepcja", "Znaczne wzmocnienie gotowania \nNiewielkie wzmocnienie przemytu i broni", "perception")
			self:CreateAttribute("willardnetworks/mainmenu/charcreation/agility.png", 38, 47, "zręczność", "Znaczne wzmocnienie przemytu i prędkości ruchu \nNiewielkie wzmocnienie medycyny", "agility")
			self:CreateAttribute("willardnetworks/mainmenu/charcreation/intelligence.png", 48, 29, "inteligencja", "Znaczne wzmocnienie medycyny i wytwarzania \nNiewielkie wzmocnienie gotowania", "intelligence")

			self:CreateRightMenuYellowTextPanel("Atrybuty są permametne, mogą być tylko wzmocnione za pomocą przedmiotów", 5)
		end
...
function PANEL:CreateSkillsButton()
	...
	self:CreateMainButton(self.skillsButton, "umiejętności", "skills")
	self.skillsButton.DoClick = function()
		...
		local panelText = textPanel:Add("DLabel")
		panelText:SetText(string.utf8upper("wybór umiejętności"))
		panelText:SetFont("MenuFontNoClamp")
		panelText:SizeToContents()
		panelText:Dock(LEFT)
		panelText:SetContentAlignment(4)
		...
		self.attributesRemaining:Dock(RIGHT)
		self.attributesRemaining:SetContentAlignment(6)
		self.attributesRemaining:SetText(self:GetPointsLeft(true)..string.utf8upper(" pozostałych punktów"))
		self.attributesRemaining:SizeToContents()

		local attributes = ix.special.list or {}

		for skillID, skillInfo in pairs(ix.skill.list) do
			if (skillInfo.uniqueID == "vort" and self.faction.name != "Vortigaunt") or skillInfo.uniqueID == "bartering" then
				continue
			end
		...
		self:CreateRightMenuYellowTextPanel("Maksymalny poziom umiejętności wynosi 50", 5)
...
function PANEL:CreateCreationTitles()
	self:CreateCreationTitle("nowa", 0, 0)
	self:CreateCreationTitle("postać", SScaleMin((0 - 5) / 3), padding)
end
...
function PANEL:CreateMainButton(parent, text, img)
	...
	parent.Paint = function(this, w, h)
		if img != nil then
			self:DrawButtonUnselected(img, this, w, h, true)
		else
	...
	parent.DoClick = function()
		...
		for _, v in pairs(selections) do
			...
			if text == "language" then
				local languageText = v != "brak" and ix.languages:FindByID(v).name or v
			...
			selectionButton.DoClick = function()
				...
				if text == "language" and v != "brak" then
...
function PANEL:CreateNextBackFinishButtons(parentBack, parentNext, parentFinish, boolNext)
	...
	parentBack:SetText(string.utf8upper("wstecz"))
	...
	parentNext:SetText(string.utf8upper("dalej"))
	...
	parentFinish:SetText(string.utf8upper("ukończ"))
...
function PANEL:CreateCharacterButton()
	...
	self:CreateMainButton(self.characterButton, "postać", "character")
	self.characterButton.DoClick = function()
		...
		self:CreateRightMenuTextPanel(self.faction:GetNoGender(LocalPlayer()) != true and "imię i nazwisko/płeć" or "imię i nazwisko", 60)

		-- Name
		local nameGenderPanel = self.rightCreation:Add("DTextEntry")
		self:CreateRightMenuTextEntry(nameGenderPanel, "Wprowadź tutaj imię i nazwisko postaci...", 36, false, 35, "name")
		...
		local namewarning = self:CreateRightMenuYellowTextPanel("Imię i nazwisko postaci musi składać się z co najmniej "..minLength.." znaków ("..string.len(self.payload.name or "").." / "..maxLength..")", -1)

		nameGenderPanel.OnTextChanged = function(this)
			self.payload:Set("name", this:GetValue())
			self:CheckIfFinished()

			namewarning:SetText("Imię i nazwisko postaci muszą składać się z co najmniej "..minLength.." znaków ("..string.len(this:GetValue() or "").." / "..maxLength..")")
			namewarning:SizeToContents()
		end

		if self.faction:GetNoGenetics(LocalPlayer()) != true then
			-- Genetics
			self:CreateRightMenuTextPanel("opis cech genetycznych", 10)
			...
			if self.faction.name == "Vortigaunt" then
				self:CreateSelectionMenu("wiek", ageComboBox, 148, "age", {"młodzik (0 - 100)", "młodzian (100 - 500)", "dorosły (500 - 1000)", "mędrzec (1000 - 1500)", "starszy (1500 - 2000)"})
			else
				self:CreateSelectionMenu("wiek", ageComboBox, 100, "age", {"młody dorosły", "dorosły", "w średnim wieku", "starszy"})
			end

			local heightComboBox = comboBoxPanel:Add("DButton")

			if self.faction.name == "Civil Protection" then
				self:CreateSelectionMenu("wzrost", heightComboBox, 90, "height", {"5'5\"", "5'6\"", "5'8\"", "5'10\"", "6'0\"", "6'2\""})
			elseif self.faction.name == "Vortigaunt" then
				self:CreateSelectionMenu("wzrost", heightComboBox, 148, "height", {"6'4\"", "6'5\"", "6'6\"", "6'7\"", "6'8\"", "6'9\"", "6'10\"", "6'11\""})
			else
				self:CreateSelectionMenu("wzrost", heightComboBox, 90, "height", {"5'2\"", "5'4\"", "5'6\"", "5'8\"", "5'10\"", "6'0\"", "6'2\""})
			end

			local eyeColorBox = comboBoxPanel:Add("DButton")
			if self.faction.name == "Vortigaunt" then
				self:CreateSelectionMenu("kolor oczu", eyeColorBox, 146, "eye color", {"czerwone", "żółte", "pomarańczowe"})
			else
				self:CreateSelectionMenu("kolor oczu", eyeColorBox, 110, "eye color", {"niebieskie", "zielone", "brązowe", "piwne", "bursztynowe", "szare"})
			end

			self:CreateRightMenuYellowTextPanel("Wartości opisu cech genetycznych są permanentne.", 10)
		end
		...
		if self.faction.name != "Overwatch AI" then
			...
			table.insert(languages, "brak")

			self:CreateRightMenuTextPanel("drugi język", 10)
			...
			self:CreateSelectionMenu("język", languageComboBox, 110, "language", languages)
		end

		-- Description
		self:CreateRightMenuTextPanel("opis fizyczny postaci", 20)
		local charDescPanel = self.rightCreation:Add("DTextEntry")
		self:CreateRightMenuTextEntry(charDescPanel, "Wprowadź tutaj opis fizyczny swojej postaci...", 130, true, 1000, "desc")

		local minDescLength = ix.config.Get("minDescriptionLength", 16)
		local maxDescLength = ix.config.Get("maxDescriptionLength", 512)
		local descLength = self:CreateRightMenuYellowTextPanel("Opis musi składać się z co najmniej "..minDescLength.." znaków ("..string.len(self.payload.description or "").." / "..maxDescLength..")", -1)

		charDescPanel.OnTextChanged = function(this)
			self.payload:Set("description", this:GetValue())
			self:CheckIfFinished()

			descLength:SetText("Opis musi składać się z co najmniej "..minDescLength.." znaków ("..string.len(this:GetValue()).." / "..maxDescLength..")")
...
function PANEL:RefreshSkins()
	...
	if self.glassesButtonPanel and IsValid(self.glassesButtonPanel) then
		for i = 1, 2 do
			...
			if i == 1 then
				glassesButton:SetText("NIE")
			else
				glassesButton:SetText("TAK")
			end
...
function PANEL:CreateAppearancesButton()
	...
	self:CreateMainButton(self.appearancesButton, "wygląd", "appearances")
	...
	self.appearancesButton.DoClick = function()
		...
		self:CreateRightMenuTextPanel("wybór modelu", 60)
		...
		self:CreateRightMenuYellowTextPanel("Wybrany model postaci jest permanentny.", 10)

		if faction:GetNoAppearances(LocalPlayer()) != true then
			self:CreateRightMenuTextPanel((faction.index == FACTION_VORT and "kolor skóry/zmarszczki" or "rysy twarzy"), 10)
			...
			if faction.index != FACTION_VORT then
				self:CreateRightMenuTextPanel("Czy postać potrzebuje okularów?", 20)

				self.glassesButtonPanel = self.rightCreation:Add("Panel")
				self:CreateButtonPanel(self.glassesButtonPanel)

				self:CreateRightMenuYellowTextPanel("Opcja 'TAK' oznacza, że obraz bez okularów będzie rozmazany.", 10)
				end
		...
		if !faction.ReadOptionDisabled then
			self:CreateRightMenuTextPanel("Czy postać umie czytać?", 5)
			...
			for i = 1, 2 do
				...
				if i == 1 then
					canreadButton:SetText("NIE")
				else
					canreadButton:SetText("TAK")
				end
			...
			self:CreateRightMenuYellowTextPanel("Opcja 'NIE' oznacza, że litery m.in. w książkach będą w zmienionej kolejności.", 10)
		end
...
function PANEL:CreateHairButton()
	...
	self:CreateMainButton(self.hairButton, "włosy", "hair")
	self.hairButton.DoClick = function()
		...
		if self.faction and self.faction.noHair then
			self:CreateRightMenuTextPanel("Wybór włosów nie jest dostępny dla tej frakcji", 60)
			return
		end

		self:CreateRightMenuTextPanel("wybór włosów", 60)
		...
		local chooseColorText = self:CreateRightMenuTextPanel("wybierz kolor", 30)
...
function PANEL:CreateFaceButton()
	...
	self:CreateMainButton(self.faceButton, "ubrania", "clothes")
	self.faceButton.DoClick = function()
		...
		if self.faction:GetNoAppearances(LocalPlayer()) != true and self.faction.index != FACTION_VORT then
			self:CreateRightMenuTextPanel("wybór kurtki", 60)
			...
			self:CreateRightMenuTextPanel("wybór spodni", 30)
			...
			self:CreateRightMenuTextPanel("wybór butów", 30)
			...
			self:CreateRightMenuYellowTextPanel("Nową odzież można kupić w sklepach", 10)
		else
			self:CreateRightMenuTextPanel("Wygląd", 60)
			...
			noAppearancesAvailable:SetText("Wybór odzieży nie jest dostępny dla tej frakcji")
...
function PANEL:CreateBackgroundSelectionPanels(icon, iconW, iconH, title, desc, minusMargin, difficultyText)
	...
	if difficultyText then
		...
		elseif string.match(difficultyText, "To tło zaczyna bez identyfikatora CID.") then
...
function PANEL:AttributesRefresh(attribute, number, bSkill)
	...
	self.attributesRemaining:SetText(self:GetPointsLeft(bSkill)..string.utf8upper(" pozostałych punktów"))
	self.attributesRemaining:SizeToContents()
end
