	...
	tabs["inv"] = {
		...
		Create = function(info, tempCont)
			...
			for _, v in SortedPairsByMemberValue(inventory:GetItemsByBase("base_containerbags", true), "id") do
				...
				if (v.name != "Duża torba" and v.name != "Mała torba") then
					continue
				elseif (v.name == "Duża torba") then
					if (bFoundLarge) then
						continue
					else
						bFoundLarge = true
					end
				elseif (v.name == "Mała torba") then
					if (bFoundSmall) then
						continue
					else
						bFoundSmall = true
