	...
	tabs["inv"] = {
		...
		Create = function(info, tempCont)
			...
			for _, v in SortedPairsByMemberValue(inventory:GetItemsByBase("base_containerbags", true), "id") do
				...
				if (v.name != "Büyük Çanta" and v.name != "Küçük Çanta") then
					continue
				elseif (v.name == "Büyük Çanta") then
					if (bFoundLarge) then
						continue
					else
						bFoundLarge = true
					end
				elseif (v.name == "Küçük Çanta") then
					if (bFoundSmall) then
						continue
					else
						bFoundSmall = true
